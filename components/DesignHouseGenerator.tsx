...
const DesignHouseGenerator: React.FC = () => {
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            ...
            const data = {
                ...
                data: { masterPrompt: promptHistory.state, sceneCount, scenes: scenesHistory.state, noVoiceover, youtubeMeta }
            };
        ...
        const handleLoad = (e: any) => {
            ...
            if (d.masterPrompt) promptHistory.reset(d.masterPrompt);
            if (d.sceneCount) setSceneCount(d.sceneCount);
            if (d.scenes) scenesHistory.reset(d.scenes);
    ...
    }, [promptHistory, scenesHistory, sceneCount, noVoiceover, youtubeMeta]);

    const handleGenerateScript = async () => {
        if (!promptHistory.state.trim()) {
        ...
        try {
            const result = await generateConsistentStoryScript(
                `HOUSE MODEL RESTORATION ASMR PRODUCTION. Context: ${promptHistory.state}. Style: 100% Realistic, Professional Architectural Restoration Photography. Show the satisfying progress from absolute ruin to modern house perfection.${voiceInstruction}`,
                sceneCount
            );
            scenesHistory.set(result.map(s => ({ ...s, isLoadingImage: false })));
        } catch (err) {
    ...
    const handleGenerateImage = async (index: number) => {
        const scene = scenesHistory.state[index];
        if (!scene) return;

        scenesHistory.set(prev => prev.map((s, i) => i === index ? { ...s, isLoadingImage: true } : s));
        try {
            const prompt = `100% Realistic, professional architectural photography of a house renovation, 8k. Action: ${scene.action}. Setting: ${scene.consistentContext}. Detailed textures of old vs new wood, wet fresh paint, cinematic lighting, wide angle. Strictly a house model. No text. 100% architectural consistency.`;
            const url = await generateImage(prompt, '16:9');
            scenesHistory.set(prev => prev.map((s, i) => i === index ? { ...s, imageUrl: url, isLoadingImage: false } : s));
        } catch (err) {
            scenesHistory.set(prev => prev.map((s, i) => i === index ? { ...s, isLoadingImage: false } : s));
        }
    };
    ...
    const handleRenderAll = async () => {
        ...
        for (let i = 0; i < scenesHistory.state.length; i++) {
            if (stopSignal.current) break;
            if (scenesHistory.state[i].imageUrl) continue;
            sceneRefs.current[i]?.scrollIntoView({ behavior: 'smooth', block: 'center' });
            await handleGenerateImage(i);
            if (i < scenesHistory.state.length - 1) await new Promise(r => setTimeout(r, 1200));
        }
        setIsRenderingAll(false);
    };

    const handleDownloadAll = () => {
        scenesHistory.state.forEach((s, i) => {
            if (s.imageUrl) {
                setTimeout(() => {
                    const a = document.createElement('a');
                    a.href = s.imageUrl!;
                    a.download = `House_Scene_${s.sceneNumber}.png`;
                    a.click();
                }, i * 500);
            }
        });
    ...
    const handleDownload = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `House_Sense_${num}.png`;
    ...
    const handleGenerateYouTubeInfo = async () => {
        if (scenesHistory.state.length === 0) return;
        setIsGeneratingMeta(true);
        try {
            const context = `House Project: ${promptHistory.state}\nScenes:\n${scenesHistory.state.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(promptHistory.state.substring(0, 50), context, "House Restoration");
            setYoutubeMeta(meta);
        } catch (e) {
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                        <div className="flex justify-between items-center">
                            <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-600">Design House (ASMR)</h2>
                            <div className="flex gap-1">
                                <button disabled={!promptHistory.canUndo} onClick={promptHistory.undo} className="p-2 bg-gray-900 hover:bg-gray-700 rounded-lg disabled:opacity-30 transition-all"><UndoIcon /></button>
                                <button disabled={!promptHistory.canRedo} onClick={promptHistory.redo} className="p-2 bg-gray-900 hover:bg-gray-700 rounded-lg disabled:opacity-30 transition-all"><RedoIcon /></button>
                            </div>
                        </div>
                        <textarea 
                            ...
                            className="w-full bg-gray-900 border border-gray-600 rounded-xl p-4 text-white h-48 resize-none focus:ring-2 focus:ring-emerald-500 outline-none shadow-inner text-sm"
                        />
                        <button onClick={handleGenerateScript} disabled={isGeneratingScript} className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-sm">
                ...
                <div className="lg:col-span-8 space-y-6">
                    {scenesHistory.state.length > 0 && (
                        <div className="bg-gray-800/80 p-4 rounded-2xl border border-gray-700 shadow-xl flex flex-wrap justify-between items-center gap-4 sticky top-0 z-20 backdrop-blur-md">
                            <div className="flex items-center gap-4">
                                <h3 className="text-xl font-black uppercase tracking-tighter">Storyboard ({scenesHistory.state.length})</h3>
                                <div className="flex gap-1">
                                    <button disabled={!scenesHistory.canUndo} onClick={scenesHistory.undo} className="p-2 bg-gray-900 hover:bg-gray-700 rounded-lg disabled:opacity-30 transition-all"><UndoIcon /></button>
                                    <button disabled={!scenesHistory.canRedo} onClick={scenesHistory.redo} className="p-2 bg-gray-900 hover:bg-gray-700 rounded-lg disabled:opacity-30 transition-all"><RedoIcon /></button>
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[1200px] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        {scenesHistory.state.map((scene, idx) => (
                            <div key={idx} ref={(el) => (sceneRefs.current[idx] = el)} className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col group hover:border-emerald-500/50 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        <img src={scene.imageUrl} className="w-full h-full object-cover" alt="Render" />
                                    ) : (
                                        <div className="flex flex-col items-center gap-2">
                                            {scene.isLoadingImage ? <Spinner className="h-6 w-6 text-emerald-500" /> : <button onClick={() => handleGenerateImage(idx)} className="px-3 py-1 bg-gray-800 text-[10px] text-gray-400 hover:text-white rounded uppercase font-bold">Render Art</button>}
                                        </div>
                                    )}
