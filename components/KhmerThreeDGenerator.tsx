...
Constraint: Please keep the characters’ faces and characteristics exactly the same in every scene. Do NOT change the characters’ faces or introduce new places. Please maintain everything exactly the same from Scene 1 all the way to Scene 100 — keep the characters’ faces and characteristics consistent in every scene.`;
        ...
        try {
            await withRetry(async () => {
                ...
                const prompt = `
                    ...
                    Create a highly detailed multi-scene animation script in 100% PURE KHMER LANGUAGE based on: "${topic}".
    ...
    const handlePlayAudio = async (sceneNumber: number, dialogIndex: number, text: string) => {
        // COMMENT: Fixed missing 'key' variable definition.
    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-4">
            <ClearProjectButton onClick={handleClearProject} />
            ...
            {/* Added project reloading history section */}
            <div className="mt-8 flex flex-col items-center">
                <button 
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-xl transition-all duration-300 border shadow-lg ${showHistory ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-800'}`}
                >
                    <HistoryIcon /> {showHistory ? 'Hide History' : 'Reload History | ប្រវត្តផលិត'}
                </button>
                {showHistory && (
                    <div className="w-full bg-[#0f172a]/95 border-2 border-indigo-500/50 p-6 rounded-3xl mt-4 animate-slide-down shadow-[0_0_50px_rgba(79,70,229,0.2)] relative z-20 backdrop-blur-xl">
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="text-lg font-black text-indigo-400 uppercase tracking-widest flex items-center gap-3">
                                <HistoryIcon className="h-5 w-5" /> 3D History Vault
                            </h4>
                            ...
                            {localHistory.length > 0 ? (
                                localHistory.map((project, idx) => (
                                    ...
                                    >
                                        ...
                                        <p className="text-white font-bold truncate mb-1">{project.data.topic || "Untitled Project"}</p>
