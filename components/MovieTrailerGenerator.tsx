...
// FIX: Added missing YouTubeIcon component definition.
...
const MovieTrailerGenerator: React.FC = () => {
    ...
    const handleGetVoiceoverText = async () => {
        ...
        try {
            const script = await generateHollywoodNarration(topic, style);
    ...
    const handleGetMusicPrompt = async () => {
        ...
        setIsGeneratingExtras(true);
        try {
            // FIX: Corrected call to implemented generateHollywoodMusicPrompt.
            const prompt = await generateHollywoodMusicPrompt(topic, style);
    ...
    const handleGetYouTubeMetadata = async () => {
        if (!topic.trim() && scenes.length === 0) return;
        setIsGeneratingMetadata(true);
        try {
            const context = `Story: ${topic}\n\nStoryboard:\n${scenes.map(s => s.description).join('\n')}`;
            const meta = await generateYouTubeMetadata(topic.substring(0, 50), context, 'Movie Trailer');
            setMetadata(meta);
        } catch (err) {
    ...
    const handleClear = () => {
        ...
        setMetadata(null);
        setCharCount(1);
        setCharacters([{ id: 1, name: 'Main Character', description: '', image: null, isAnalyzing: false }]);
                        ...
                        <>
                            ...
                            <div>
                                ...
                                <div className="flex justify-between items-center bg-gray-900 p-2 rounded border border-gray-700">
                                    <span className="text-xs text-gray-500 font-bold">Start: 25</span>
                                    <span className="text-xl font-black text-red-500">{targetSceneCount} Senses</span>
                         ...
                         <div className="space-y-6 animate-fade-in text-center p-8 text-gray-500 italic">
                             Feature available in Trailer mode.
                         </div>
                    )}
                    {error && <div className="p-3 bg-red-900/50 border border-red-700 text-red-200 rounded text-center text-sm">{error}</div>}
                </div>

                {/* Right: Storyboard Output */}
                <div className="lg:col-span-2 space-y-6">
                    {/* Improved YouTube Kit Section */}
                    {scenes.length > 0 && (
                        <div className="bg-gray-800/60 p-6 rounded-2xl border border-red-500/30 shadow-2xl relative overflow-hidden animate-fade-in">
                            <div className="absolute top-0 left-0 w-1 h-full bg-red-600"></div>
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                                <h3 className="text-lg font-black text-red-500 flex items-center gap-3 uppercase tracking-tighter"><YouTubeIcon /> TRAILER DISTRIBUTION KIT</h3>
                                <div className="flex gap-2">
                                    {metadata && (
                                        <button 
                                            onClick={() => {
                                                const fullPackage = `TITLE:\n${metadata.title}\n\nDESCRIPTION:\n${metadata.description}\n\nHASHTAGS:\n${metadata.hashtags.join(' ')}\n\nTAGS:\n${metadata.keywords.join(', ')}`;
                                                handleCopyText(fullPackage, 'full-package');
                                            }} 
                                            className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white text-[10px] font-black uppercase rounded-xl border border-gray-600 transition flex items-center gap-2 shadow-sm"
                                        >
                                            {copyStatus === 'full-package' ? <CheckIcon /> : <CopyIcon />} Copy All Meta
                                        </button>
                                    )}
                                    <button onClick={handleGetYouTubeMetadata} disabled={isGeneratingMetadata} className="px-5 py-2 bg-red-900/20 hover:bg-red-900/40 text-red-400 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all border border-red-800/50 shadow-lg">
                                        {isGeneratingMetadata ? <Spinner className="h-3 w-3" /> : 'GENERATE SEO INFO'}
                                    </button>
                                </div>
                            </div>
                            ...
                            {metadata ? (
                                <div className="space-y-6">
                                    <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700">
                                        <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest">
                                            ...
                                            <button onClick={() => handleCopyText(metadata.title, 'title')} className="hover:text-white transition-colors">{copyStatus === 'title' ? '✓' : 'COPY'}</button>
                                        </div>
                                    ...
                                    <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700">
                                        <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest">
                                            ...
                                            <button onClick={() => handleCopyText(metadata.description, 'desc')} className="hover:text-white transition-colors">{copyStatus === 'desc' ? '✓' : 'COPY'}</button>
                                        </div>
                                        <div className="text-gray-300 text-[10px] whitespace-pre-wrap max-h-32 overflow-y-auto custom-scrollbar font-serif leading-relaxed italic">{metadata.description}</div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700">
                                            <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest">
                                                <span>HASHTAGS</span>
                                                <button onClick={() => handleCopyText(metadata.hashtags.join(' '), 'tags')} className="hover:text-white transition-colors">{copyStatus === 'tags' ? '✓' : 'COPY'}</button>
                                            </div>
                                            <div className="text-blue-400 text-[10px] font-bold font-mono">{metadata.hashtags.join(' ')}</div>
                                        </div>
                                        <div className="bg-gray-900/60 p-4 rounded-xl border border-gray-700">
                                            <div className="flex justify-between text-[9px] font-black text-gray-500 mb-2 tracking-widest">
                                                <span className="flex items-center gap-2">
                                                ...
                                                <button onClick={() => handleCopyText(tagsValue, 'keywords')} className="hover:text-white transition-colors">{copyStatus === 'keywords' ? '✓' : 'COPY'}</button>
                                            </div>
                                            <div className={`text-cyan-400 text-[9px] font-medium font-mono leading-relaxed ${isTagsOver ? 'text-red-400' : ''}`}>
                                                {tagsValue}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <p className="text-center text-gray-600 text-[10px] py-4 font-bold uppercase tracking-widest opacity-40">Architect a trailer to unlock distribution assets.</p>
                            )}
                        </div>
                    )}

                    <div className="bg-gray-800/60 p-4 rounded-xl border border-gray-700 shadow-2xl">
                        ...
                        {isLoading && scenes.length === 0 && (
                             <div className="h-64 flex flex-col items-center justify-center p-8 bg-gray-900/50 rounded-xl border border-gray-700 animate-fade-in">
                                <div className="text-3xl font-black text-white mb-6 animate-pulse">
                                    PRODUCTION: {progress}%
                                </div>
                                ...
                                <p className="text-xs text-cyan-400 mt-6 uppercase tracking-[0.3em] font-black">Directing Blockbuster Sequence...</p>
                            </div>
                        )}

                        <div className="space-y-6 max-h-[85vh] overflow-y-auto custom-scrollbar pr-2 pb-20">
                            {scenes.map((scene, idx) => (
                                <div key={idx} className="bg-gray-900 p-5 rounded-2xl border border-gray-700 shadow-xl hover:border-red-500/30 transition-all group">
                                    ...
                                    <p className="text-gray-300 text-sm mb-4 font-serif italic leading-relaxed border-l-2 border-yellow-500 pl-4 bg-white/5 py-2 rounded-r-lg">
                                        "{scene.description}"
                                    </p>
                                    
                                    <div className="bg-black/40 p-4 rounded-xl border border-gray-800 mb-4">
                                        <p className="text-[9px] font-black text-gray-500 uppercase mb-2 tracking-widest">Rendering Prompt</p>
                                        <p className="text-gray-400 text-[11px] font-mono leading-relaxed">{scene.video_prompt}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
