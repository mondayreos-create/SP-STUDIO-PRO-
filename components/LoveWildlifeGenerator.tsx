...
const LoveWildlifeGenerator: React.FC = () => {
    const [animalName, setAnimalName] = useState('');
    const [sensesCount, setSensesCount] = useState(10);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (includeCameraAngles) {
                ...
                For EACH "Sense" generated, strictly assign a unique CINEMATIC CAMERA SHOT/ANGLE for high-fidelity video generation. 
                Use: Wide Shot, Medium Shot, Close-up (Macro Detail), POV, Low Angle, B-Roll, and Movement (Pan/Tilt/Dolly).
            ...
            const prompt = `
                Generate exactly ${sensesCount} high-quality, 100% photorealistic video generation prompts for the animal: "${animalName}".
                
                MANDATORY STYLE KEYWORDS:
                "100% Realistic, National Geographic style, shot 1080, 2k, 4k, 8k, raw photo, cinematic lighting, macro details."
                
                ${cameraContext}

                REQUIREMENTS:
                1. Subject: ${animalName} in its natural habitat.
                2. Senses: Create exactly ${sensesCount} distinct video generation prompts.
                
                OUTPUT JSON FORMAT:
                {
                    "mainDescription": "The core visual description of the animal and environment for a professional 8K documentary video.",
                    "senses": [
    ...
    const handleRegenerateSense = async (id: number) => {
        ...
        try {
            const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
            const prompt = `Generate a new 100% realistic video generation prompt for Sense #${id} of a "${animalName}" wildlife film. Focus on high-end cinematic detail.`;
        ...
        <div className="w-full max-w-5xl mx-auto p-4 flex flex-col items-center">
            ...
            <div className="w-full bg-gray-800/60 p-8 rounded-xl border border-gray-700 shadow-2xl">
                <div className="text-center mb-8">
                    <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-600 mb-2 flex items-center justify-center gap-2">
                        <span>🦁</span> Love Wildlife
                    </h2>
                    <p className="text-gray-400 text-sm">Generate 100% realistic video prompts for professional wildlife filmmaking.</p>
                </div>

                <div className="bg-gray-900/50 p-6 rounded-lg border border-gray-700 mb-8">
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                        <div className="md:col-span-5">
                            <label className="block text-xs font-semibold text-gray-400 mb-1">Animal Name</label>
                            <input type="text" value={animalName} onChange={(e) => setAnimalName(e.target.value)} placeholder="e.g. Majestic Lion..." className="w-full bg-gray-800 border border-gray-600 rounded-lg p-3 text-white outline-none" />
                        </div>
                        <div className="md:col-span-2">
                            <label className="block text-xs font-semibold text-gray-400 mb-1">Senses Qty</label>
                            <input type="number" min="1" max="900" value={sensesCount} onChange={(e) => setSensesCount(Math.max(1, Math.min(900, parseInt(e.target.value) || 1)))} className="w-full bg-gray-800 border border-gray-600 rounded-lg p-3 text-white outline-none" />
                        </div>
                        <div className="md:col-span-5 flex flex-col pt-1 gap-2">
                            <label className="flex items-center gap-2 cursor-pointer bg-gray-800 border border-gray-600 rounded-lg p-2.5">
                                ...
                                <span className="text-xs font-bold text-purple-400 flex items-center gap-1"><VideoCameraIcon /> Cinematic Shots</span>
                            </label>
                            <button onClick={handleGenerate} disabled={isLoading || !animalName.trim()} className="w-full py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-bold rounded-lg shadow-lg active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                {content && (
                    <div className="space-y-6 animate-fade-in">
                        <div className="bg-gray-900/80 p-4 rounded-xl border border-gray-700 backdrop-blur-md sticky top-0 z-10 flex justify-between items-center shadow-xl">
                            <span className="text-xs font-bold text-green-400 uppercase tracking-widest">Production Results</span>
                        ...
                        <div className="space-y-4">
                            {content.senses.map((sense) => (
                                <div key={sense.id} className="bg-gray-800 p-4 rounded-lg border border-gray-700 hover:border-green-500/50 transition-all duration-300">
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        <div className="flex-grow">
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className="bg-green-900/30 text-green-400 text-xs font-black px-2 py-0.5 rounded border border-green-800">SENSE {sense.id}</span>
                                            </div>
                                            {sense.isEditing ? (
...
export default LoveWildlifeGenerator;
