...
const factorySpins = [
    { label: "Frozen Pizza Factory", icon: "🍕", prompt: "Frozen Pizza MEGA FACTORY: How Automation Produces TONS of Pizzas Daily. Focus on high-speed industrial assembly, dough spreading, sauce application robots, waterfall of cheese, and flash freezing tunnel. 100% Realistic, satisfyning mechanical flow." },
    { label: "Auto Assembly Line", icon: "🚗", prompt: "High-Speed Car Production: Robotic arms welding, painting, and assembling luxury electric vehicles. Satisfying sparks, glossy paint finishes, and precision mechanical choreography. 8k cinematic lighting." },
    { label: "Soft Drink Plant", icon: "🥤", prompt: "Hydraulic Soda Production: Millions of aluminum cans moving through high-pressure rinsing, filling at 2000 cans/min, and laser-speed capping. Liquid splashes in macro detail, industrial ASMR vibes." },
    { label: "Chocolate Production", icon: "🍫", prompt: "Artisan Chocolate Factory: Rhythmic tempering of dark chocolate, liquid waterfalls of cocoa, automated molding into identical bars, and high-speed foil wrapping." },
    { label: "Potato Chip Factory", icon: "🥔", prompt: "Massive Potato Chip Production: High-speed slicing, deep frying in golden oil, automated seasoning spray, and high-speed packaging into bags. 100% satisfying crunch visuals." },
    { label: "Glass Bottle Factory", icon: "🍾", prompt: "Glowing Glass Bottle Production: Molten glass being blown into molds, red-hot glowing bottles moving on conveyors through cooling zones. Atmospheric industrial lighting." }
];
...
const ProductionProcessGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [masterPrompt, setMasterPrompt] = useState(factorySpins[0].prompt);
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            if (e.detail.tool !== 'production-process') return;
            const data = {
                id: crypto.randomUUID(),
                timestamp: Date.now(),
                tool: 'production-process',
                category: 'vip',
                title: "Production Line Process",
    ...
    const handleSelectSpin = (opt: typeof factorySpins[0]) => {
    ...
    const handleGenerateScript = async () => {
        if (!masterPrompt.trim()) {
            setError("Please enter a production concept.");
            return;
        }
        ...
        const voiceInstruction = noVoiceover ? "\n\nSTRICT RULE: Do NOT include any spoken dialogue or narration. Focus purely on environmental ASMR sounds: mechanical hums, rhythmic clicking, and liquid movement." : "";

        try {
            const result = await generateConsistentStoryScript(
                `PRODUCTION LINE PROCESS SCRIPT. Context: ${masterPrompt}. 
                STYLE: 100% Realistic, Professional Cinematic Industrial Photography. 
                Focus on the repetitive, satisfying flow of a modern high-speed factory. 
                Show the progression from raw material entry -> automated processing -> quality check -> final packaging.${voiceInstruction}`,
                sceneCount
            );
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic, professional industrial cinematography, 8k. Action: ${scene.action}. Setting: ${scene.consistentContext}. Hypnotic repeating patterns, stainless steel, steam, vibrant factory lighting, macro focus on movement.`;
    ...
    const handleCopyJson = (index: number) => {
        ...
        const structuredData = {
            ...
            prompt: `100% Realistic, industrial production photography, 8k. Action: ${scene.action}. Context: ${scene.consistentContext}.`
        };
    ...
    const handleGenerateMetadata = async () => {
        ...
        setError(null);
        try {
            const context = `Industrial Process: ${masterPrompt}\n\nProduction Senses:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(
                "Most Satisfying Industrial Production Line | Modern Factory Process",
                context,
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `Production_Process_Sense_${num}.png`;
    ...
    const handleClear = () => {
        setMasterPrompt(factorySpins[0].prompt);
        setScenes([]);
        setNoVoiceover(true);
        setYoutubeMeta(null);
        setError(null);
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in">
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-500 mb-2 flex items-center gap-2">
                        <span>⚙️</span> ខ្សែសង្វាក់ផលិតកម្ម
                    </h2>
                    ...
                    <button 
                        ...
                        className="w-full py-4 bg-gradient-to-r from-blue-600 to-cyan-700 hover:brightness-110 text-white font-black rounded-xl shadow-xl transition transform active:scale-[0.98] disabled:opacity-50 text-lg flex items-center justify-center gap-3 uppercase tracking-widest"
                    >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[85vh] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        ...
                        {scenes.length === 0 && !isGeneratingScript && (
                            <div className="col-span-full py-20 text-center bg-gray-900/20 rounded-[3rem] border-4 border-dashed border-gray-800 flex flex-col items-center justify-center">
                                 <div className="text-8xl mb-4 opacity-10 grayscale">⚙️</div>
                                 <p className="text-xl font-black text-gray-600 uppercase tracking-[0.4em]">Architect floor ready</p>
                                 <p className="text-sm text-gray-700 mt-4 max-w-md">Select a theme or describe your industrial process and click "Get Sense Script" to begin.</p>
                            </div>
                        )}
                    </div>
                </div>
...
export default ProductionProcessGenerator;
