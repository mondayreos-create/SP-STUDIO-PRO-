...
const CompanyProductGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [masterPrompt, setMasterPrompt] = useState('Industrial Production: Hypnotic flow of a modern automated factory. Thousands of items moving through robotic pressing, high-speed bottling, and final bulk shipping. Focus on satisfying repetitive motions, steam, and steel textures.');
    ...
    const handleGenerateScript = async () => {
        ...
        try {
            const result = await generateConsistentStoryScript(
                `INDUSTRIAL PRODUCTION FLOW. Context: ${masterPrompt}. Style: 100% Realistic, Professional Cinematic Industrial Photography. Focus on hypnotic, repetitive machine movement.`,
                sceneCount
            );
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic, professional industrial cinematography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Hypnotic repeating patterns, steam, vibrant factory lighting, 100% consistent machines. No text.`;
        ...
        scenes.forEach((s, i) => {
            if (s.imageUrl) {
                setTimeout(() => {
                    ...
                    a.href = s.imageUrl!;
                    a.download = `Factory_Scene_${s.sceneNumber}.png`;
                    a.click();
                }, i * 500);
            }
        });
    ...
    const handleDownload = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `Factory_Sense_${num}.png`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    };

    // FIX: Added missing handleClear function.
    const handleClear = () => {
        setMasterPrompt('Industrial Production: Hypnotic flow of a modern automated factory. Thousands of items moving through robotic pressing, high-speed bottling, and final bulk shipping. Focus on satisfying repetitive motions, steam, and steel textures.');
        setScenes([]);
        setYoutubeMeta(null);
        setError(null);
    };
    ...
    const handleGenerateYouTubeInfo = async () => {
        ...
        try {
            const context = `Industrial Project: ${masterPrompt}\nSenses:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(masterPrompt.substring(0, 50), context, "Industrial Production");
            setYoutubeMeta(meta);
        } catch (e) {
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                        <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-slate-400">Production Line Pro</h2>
                        <textarea 
                            value={masterPrompt}
                            onChange={(e) => setMasterPrompt(e.target.value)}
                            placeholder="Describe the production flow..."
                            className="w-full bg-gray-900 border border-gray-600 rounded-xl p-4 text-white h-48 resize-none focus:ring-2 focus:ring-blue-500 outline-none shadow-inner text-sm"
                        />
                        <button onClick={handleGenerateScript} disabled={isGeneratingScript} className="w-full py-4 bg-gradient-to-r from-blue-600 to-slate-600 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-sm">
                            {isGeneratingScript ? <Spinner /> : 'Architect Factory'}
                        </button>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    {youtubeMeta && (
                        <div className="bg-gray-900 p-6 rounded-2xl border border-red-500/30 animate-fade-in space-y-4 shadow-2xl relative overflow-hidden">
                            ...
                            <div className="space-y-4 text-xs">
                                ...
                                <div><label className="text-gray-500 uppercase mb-1 block">Description</label><div className="bg-black/40 p-3 rounded-lg border border-gray-800 text-gray-300 whitespace-pre-wrap italic font-serif">{youtubeMeta.description}</div></div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[1200px] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        {scenes.map((scene, idx) => (
                            <div key={idx} ref={(el) => (sceneRefs.current[idx] = el)} className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col group hover:border-blue-500/50 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        <img src={scene.imageUrl} className="w-full h-full object-cover" alt="Render" />
                                    ) : (
                                        <div className="flex flex-col items-center gap-2">
                                            {scene.isLoadingImage ? <Spinner className="h-6 w-6 text-blue-500" /> : <button onClick={() => handleGenerateImage(idx)} className="px-3 py-1 bg-gray-800 text-[10px] text-gray-400 hover:text-white rounded uppercase font-bold">Render Art</button>}
                                        </div>
                                    )}
