...
const presetGroups = [
    {
        name: "ផ្នែកទី ១: Infrastructure & Construction (ស្ពាន និង ផ្លូវថ្នល់)",
        options: [
            { label: 'Suspension Bridge', value: 'Suspension Bridge', desc: "Construction site of a massive suspension bridge connecting two mountains over a deep river, heavy cranes lifting steel beams, workers in safety gear, golden hour sunlight, hyper-realistic, 8k resolution, cinematic angle." },
            { label: 'Highway Asphalting', value: 'Highway Asphalting', desc: "Road construction crew laying fresh black asphalt on a highway, steam rising from hot tar, heavy road rollers flattening the surface, aerial view, realistic texture, highly detailed, 4k." },
            { label: 'City Flyover Night', value: 'City Flyover', desc: "Night construction of a modern city flyover, welders working with sparks flying, long exposure of car lights below, industrial atmosphere, sharp focus, cinematic lighting, 8k." },
            { label: 'Mountain Tunnel', value: 'Mountain Tunnel', desc: "Engineering marvel of a tunnel boring machine breaking through a mountain rock, dust and debris, bright industrial lights illuminating the tunnel, realistic construction site, wide angle shot." }
        ]
    },
    {
        name: "ផ្នែកទី ២: House Building (ការសាងសង់ផ្ទះ)",
        options: [
            { label: 'Villa Foundation', value: 'Villa Foundation', desc: "Construction site of a luxury modern villa, workers pouring concrete into foundation molds, architects reviewing blueprints, sunny day with blue sky, 100% realistic, architectural photography style." },
            { label: 'Wooden Cabin Frame', value: 'Wooden Cabin', desc: "Building a wooden cabin in a pine forest, carpentry framing stage, raw wood texture, sunlight filtering through trees, sawdust in the air, cozy atmosphere, photorealistic, 8k." },
            { label: 'Glass House', value: 'Glass House', desc: "Installation of large floor-to-ceiling glass windows on a modern minimalist house, steel structure visible, reflective glass, construction tools on the ground, high-end architecture, clean lines." }
        ]
    },
    {
        name: "ផ្នែកទី ៣: Interior Design (ការរៀបចំបន្ទប់)",
        options: [
            { label: 'Modern Living Room', value: 'Modern Living Room', desc: "Interior design of a cozy living room, Scandinavian style, beige sofa, warm lighting from floor lamps, indoor plants, marble coffee table, highly detailed fabric texture, 4k, architectural digest style." },
            { label: 'Minimalist Bedroom', value: 'Minimalist Bedroom', desc: "A master bedroom with a large window overlooking the ocean, king-size bed with white linens, soft morning light, wooden flooring, zen atmosphere, photorealistic, interior design photography." },
            { label: 'Cyberpunk Workspace', value: 'Cyberpunk Room', desc: "Ultimate home office and gaming setup, triple monitor display, neon RGB lighting strips on the wall, ergonomic chair, soundproofing foam, high-tech vibe, cinematic shot, 8k." }
        ]
    }
];

const BuildingDitGenerator: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (includeCameraAngles) {
                cameraContext = `
                CRITICAL VISUAL INSTRUCTION (ARCHITECTURAL SHOTS):
                For EACH "Sense" generated, you MUST strictly assign a unique ARCHITECTURAL CAMERA SHOT/ANGLE. 
                Vary them to create a professional sequence. Use these specifically:
                1. Establishing Shot / Wide Shot (To show the full building/structure)
                2. Medium Shot (To show entrance, facade details, or specific sections)
                3. Close-up (Focus on textures, materials, windows, patterns - "Detail")
                4. Low Angle (To make the building look tall, imposing, and majestic)
                5. High Angle / Drone Shot (To show the layout and surrounding context)
                6. Interior Shot (If applicable, showing light, space, and design)
                7. Isometric View (For structural clarity)
            ...
            const prompt = `
                Generate a highly detailed, 100% photorealistic architectural image prompt for the building/structure: "${buildingName}".
                
                ${contextPrompt ? `CONTEXT DESCRIPTION:\n"${contextPrompt}"\nUse this detailed context to inform the visual elements.\n` : ''}

                MANDATORY STYLE KEYWORDS:
                "100% Realistic, realistic, shot 1080, 2k, 4k, 8k, architectural photography, hyper-detailed, cinematic lighting, sharp focus."
                
                ${cameraContext}

                REQUIREMENTS:
                1. Subject: ${buildingName} (Focus on Architecture, Design, Interior/Exterior elements).
                2. Senses: Create exactly ${sensesCount} distinct sensory details (visual, texture, light, atmosphere, scale).
    ...
    const handleGenerateMetadata = async () => {
        ...
        try {
            ...
            const meta = await generateYouTubeMetadata(
                `Cinematic ${buildingName} Tour 4K`, 
                context, 
                'Architecture'
            );
            setYoutubeMeta(meta);
    ...
    const handleRegenerateSense = async (id: number) => {
        ...
        try {
            ...
            let cameraPrompt = "";
            if (includeCameraAngles) {
                cameraPrompt = "Include a specific ARCHITECTURAL CAMERA SHOT (e.g., Low Angle, Drone Shot, Close-up Texture) at the beginning.";
            }

            const prompt = `
                Context: A 100% realistic, 8k architectural image prompt for "${buildingName}".
                Main Description: ${content.mainDescription}
                
                Task: Rewrite or generate a NEW detailed "Sense" description (Sense #${id}) focusing on a specific detail (material texture, lighting shadow, structural line, scale).
                ${cameraPrompt}
        ...
        <div className="w-full max-w-5xl mx-auto p-4 flex flex-col items-center">
            ...
            <div className="w-full bg-gray-800/60 p-8 rounded-xl border border-gray-700 shadow-2xl">
                <div className="text-center mb-8">
                    <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-amber-500 mb-2 flex items-center justify-center gap-2">
                        <span>🏗️</span> Building DIT
                    </h2>
                    <p className="text-gray-400 text-sm">Generate 100% realistic architectural & structure prompts.</p>
                </div>
                ...
                <div className="bg-gray-900/50 p-6 rounded-lg border border-gray-700 mb-8">
                    {/* Preset Selector */}
                    <div className="mb-6">
                        <label className="block text-xs font-semibold text-amber-400 mb-2 uppercase tracking-wider">Choose Blueprint / Scenario</label>
                        <select 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                        <div className="md:col-span-5">
                            <label className="block text-xs font-semibold text-gray-400 mb-1">Building / Structure Name</label>
                            <input 
                                type="text" 
                                value={buildingName} 
                                onChange={(e) => setBuildingName(e.target.value)} 
                                placeholder="e.g. Modern Glass House, Ancient Temple..." 
                        ...
                        <div className="md:col-span-5 flex flex-col justify-end h-full pt-1 gap-2">
                            ...
                            <button 
                                ...
                                className="w-full py-3 bg-gradient-to-r from-slate-600 to-amber-600 hover:from-slate-500 hover:to-amber-500 text-white font-bold rounded-lg shadow-lg transition transform active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                            >
...
export default BuildingDitGenerator;
