...
const ancientSpins = [
    { label: "Savanna Battle", icon: "🦖", prompt: "Life Millions of Years Ago: The Most Dangerous Fight Ever Seen in the Ancient Savanna. A T-Rex battling a pack of raptors during a dust storm. 100% Realistic, high-fidelity textures, cinematic lighting, National Geographic style." },
    { label: "Ice Age Mammoth", icon: "🐘", prompt: "Ice Age Survival: A massive Woolly Mammoth navigating a lethal blizzard in the snowy peaks. Ancient hunters in the distance. Photorealistic, extreme cold atmosphere, 8k documentary style." },
    { label: "Deep Ocean Terror", icon: "🦈", prompt: "Ancient Oceans: A Megalodon hunting in the dark deep sea. Bioluminescent jellyfish lighting the water, realistic bubbles, intense scale, cinematic underwater 4K." },
    { label: "Volcanic Eruption", icon: "🌋", prompt: "Pangea Apocalypse: Pterodactyls flying over an active volcanic landscape. Molten lava, thick smoke, dramatic red lighting, realistic textures of prehistoric reptiles." }
];

interface AncientScene {
    id: number;
    title: string;
    description: string;
    prompt: string;
...
const AncientLifeGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [masterPrompt, setMasterPrompt] = useState(ancientSpins[0].prompt);
    const [senseCount, setSenseCount] = useState(12);
    const [noVoiceover, setNoVoiceover] = useState(false);
    const [scenes, setScenes] = useState<AncientScene[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [copyStatus, setCopyStatus] = useState<string | null>(null);
    const [displaySpins, setDisplaySpins] = useState(ancientSpins);

    const handleShufflePresets = () => {
        const shuffled = [...ancientSpins].sort(() => Math.random() - 0.5);
        setDisplaySpins(shuffled);
    };

    const handleArchitect = async () => {
        if (!masterPrompt.trim()) {
            setError("Please enter an ancient life concept.");
            return;
        }
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            const systemInstruction = `
                You are an Ancient Life Content Architect. 
                Generate exactly ${senseCount} Senses (scenes) for a viral documentary-style video.
                
                STRICT VISUAL STYLE: 100% Realistic, CGI photorealistic, High-End Film quality.
                THEME: Ancient Creatures, Lethal Environments, Millions of Years Ago.
                
                For each sense, provide:
                - title: Catchy name for the sense.
                - description: Internal action description.
                - prompt: A highly detailed 100% Realistic video generation prompt (National Geographic style).
                - voiceover: A short, deep documentary-style narration (max 25 words).
                
                Maintain creature and environmental consistency across all ${senseCount} scenes.
            `;
    ...
    const handleRenderScene = async (index: number) => {
        ...
        try {
            const finalPrompt = `100% Realistic Photorealistic Cinematic Render, National Geographic style. ${scene.prompt}. No text. High Detail.`;
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col animate-fade-in font-sans">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-[#1e293b]/90 p-6 rounded-[2.5rem] border border-red-500/20 shadow-2xl backdrop-blur-xl ring-1 ring-white/5">
                        <div className="flex items-center gap-3 mb-6">
                            <div className="w-12 h-12 bg-gradient-to-br from-red-600 to-amber-700 rounded-2xl flex items-center justify-center shadow-lg shadow-red-500/20 transform rotate-2">
                                <span className="text-2xl">🌋</span>
                            </div>
                            <div>
                                <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-amber-400 uppercase tracking-tighter leading-none">
                                    Ancient Life X
                                </h2>
                                <p className="text-[9px] text-gray-500 font-black uppercase tracking-[0.2em] mt-1">Millions of Years Ago Production</p>
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-black/40 rounded-2xl border border-gray-800 mb-6 shadow-inner">
                            <div className="flex justify-between items-center mb-3">
                                <label className="block text-[10px] font-black text-amber-500 uppercase tracking-widest">Past Prompt Spins Box</label>
                                <button onClick={handleShufflePresets} className="p-1 hover:rotate-180 transition-transform duration-500 text-gray-600 hover:text-amber-500"><RefreshIcon /></button>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                                {displaySpins.map((opt) => (
                                    <button 
                                        ...
                                        className={`p-2.5 rounded-xl text-[9px] font-black uppercase transition-all flex items-center gap-2 border shadow-sm ${masterPrompt === opt.prompt ? 'bg-amber-900/40 border-amber-500 text-white' : 'bg-gray-900 border-gray-700 text-gray-500 hover:border-amber-500 hover:text-gray-300'}`}
                                    >
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-[#0f172a] rounded-2xl border border-gray-800 shadow-inner">
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Prehistoric Concept</label>
                                <textarea 
                                    value={masterPrompt}
                                    onChange={(e) => setMasterPrompt(e.target.value)}
                                    placeholder="Enter your ancient world vision..."
                                ...
                                <label className="flex items-center gap-3 cursor-pointer group">
                                    <div className="relative">
                                        <input 
                                            ...
                                            className="w-5 h-5 appearance-none border border-gray-700 rounded bg-black checked:bg-red-600 transition-all cursor-pointer relative"
                                        />
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full bg-[#0f172a] border border-gray-800 rounded-2xl p-4 text-white font-black text-center text-xl focus:ring-1 focus:ring-red-500 transition-shadow shadow-inner"
                                />
                            </div>
                            ...
                            <button 
                                ...
                                className="w-full py-5 bg-gradient-to-r from-red-600 to-amber-700 text-white font-black rounded-3xl shadow-2xl transition-all transform active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 uppercase tracking-widest text-sm border-t border-white/10"
                            >
                ...
                <div className="lg:col-span-8 flex flex-col h-full min-h-[700px]">
                    <div className="bg-[#111827]/90 p-8 rounded-[3rem] border border-gray-800 shadow-2xl flex flex-col h-full backdrop-blur-md relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-center mb-8 relative z-10">
                            <div>
                                <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Ancient Storyboard</h3>
                        ...
                        <div className="flex-grow overflow-y-auto custom-scrollbar pr-2 space-y-6 relative z-10 pb-20">
                            {scenes.length > 0 ? (
                                scenes.map((scene, idx) => (
                                    <div key={idx} className="bg-gray-800/20 rounded-[2.5rem] border border-gray-700/50 p-6 group hover:border-red-500/30 transition-all shadow-xl">
                                        <div className="flex flex-col md:flex-row gap-6">
                                            ...
                                            <div className="w-full md:w-64 aspect-video md:aspect-square bg-black rounded-[1.5rem] overflow-hidden relative border border-gray-700 shrink-0 flex items-center justify-center shadow-inner">
                                                    ...
                                                    <div className="text-center p-4">
                                                        {scene.isLoading ? (
                                                            <div className="flex flex-col items-center gap-3">
                                                                <Spinner className="h-10 w-10 text-red-500 m-0" />
                                                                <span className="text-[9px] font-black text-red-400 uppercase animate-pulse">Rendering CGI...</span>
                                                            </div>
                                                        ) : (
                                                            <button 
                                                                onClick={() => handleRenderScene(idx)}
                                                                className="flex flex-col items-center gap-2 text-gray-600 hover:text-red-400 transition transform hover:scale-110"
                                                            >
                                                ...
                                                <div className="absolute top-3 left-3 bg-red-600 text-white text-[9px] font-black px-3 py-1 rounded-full uppercase shadow-lg">SENSE {scene.id}</div>
                                            </div>
                                            ...
                                            <div className="flex-grow flex flex-col justify-between py-2">
                                                <div>
                                                    ...
                                                    <p className="text-gray-300 text-xs italic leading-relaxed border-l-2 border-red-500 pl-3 py-1 mb-4">
                                                        "{scene.description}"
                                                    </p>
                                                </div>
...
export default AncientLifeGenerator;
