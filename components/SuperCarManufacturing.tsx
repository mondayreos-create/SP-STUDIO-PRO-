...
const carPresets = [
    { label: "Bugatti Chiron", prompt: "Manufacturing a Bugatti Chiron Pur Sport. Step 0%: Advanced W16 quad-turbo engine mounting on a raw carbon fiber monocoque. Step 100%: French Racing Blue masterpiece. 100% Realistic 8k." },
    { label: "Ferrari SF90", prompt: "Assembly of the Ferrari SF90 Stradale. Focus on the hybrid battery integration and the electric motor mounting on the front axle. Rosso Corsa finish. 8k realism." },
    { label: "Lamborghini Revuelto", prompt: "Production of the Lamborghini Revuelto V12 hybrid. Robotic chassis welding and the installation of the active aerodynamics system. Arancio Apodis orange." },
    { label: "Pagani Utopia", prompt: "Hand-building a Pagani Utopia. Focus on the titanium-carbon fiber weave, hand-milled aluminum interior components, and the Carbo-Titanium chassis." },
    { label: "Koenigsegg Jesko", prompt: "Manufacturing the Koenigsegg Jesko Absolut. Focus on the 9-speed Light Speed Transmission assembly and the extreme rear wing installation. White carbon finish." },
    { label: "Rimac Nevera", prompt: "High-tech assembly of the Rimac Nevera electric hypercar. Installing the liquid-cooled 120kWh battery pack and the quad-motor powertrain." },
    { label: "McLaren P1", prompt: "The McLaren P1 production line. Focus on the MonoCage carbon fiber tub and the integration of the IPAS hybrid system. Volcano Orange finish." },
    { label: "Porsche Mission X", prompt: "Manufacturing the Porsche Mission X hypercar concept. Lightweight glass dome installation and the high-performance 900V system components." },
    { label: "Lotus Evija", prompt: "Assembling the Lotus Evija electric hypercar. Focus on the aerodynamic venturi tunnels carved through the body and the 2000hp motor setup." },
    { label: "Aston Martin Valkyrie", prompt: "The Formula 1 inspired assembly of the Aston Martin Valkyrie. Raw carbon fiber bodywork, high-revving V12 engine mounting, and minimal weight components." },
    { label: "Mercedes-AMG One", prompt: "Integrating an actual F1 engine into the Mercedes-AMG One. Focus on the MGU-H and MGU-K systems and the complex choreography. Silver arrows livery." },
    { label: "Pininfarina Battista", prompt: "Crafting the Pininfarina Battista. Elegant Italian design combined with high-tech EV components. Focus on the hand-stitched leather and carbon fiber trim." },
    { label: "Zenvo Aurora", prompt: "Production of the Zenvo Aurora Agil. Focus on the quad-turbo V12 hybrid engine and the exposed carbon fiber frame. High-intensity orange glow." },
    { label: "Hennessey Venom F5", prompt: "Building the Hennessey Venom F5 for top speed. Focus on the 'Fury' V8 engine installation and the ultra-low drag carbon fiber body panels." },
    { label: "Apollo IE", prompt: "The extreme manufacturing of the Apollo Intensa Emozione. Focus on the organic carbon fiber shapes and the high-pitched V12 exhaust system assembly." },
    { label: "De Tomaso P72", prompt: "Hand-crafting the De Tomaso P72. Focus on the polished copper interior accents and the classic mid-century racing aesthetics in a modern factory." },
    { label: "Maserati MC20", prompt: "Manufacturing the Maserati MC20. Focus on the Neptune twin-turbo V6 engine with pre-chamber combustion and the butterfly door hinges." },
    { label: "Ford GT Carbon", prompt: "Assembling the Ford GT Liquid Carbon edition. Focus on the visible carbon fiber weave perfection and the flying buttress aerodynamics." },
    { label: "SSC Tuatara", prompt: "Building the SSC Tuatara speed record car. Focus on the twin-turbo V8 and the aerospace-grade carbon fiber body construction." },
    { label: "Gordon Murray T.50", prompt: "The precision engineering of the Gordon Murray T.50. Focus on the rear-mounted ground-effect fan and the central driving position assembly." }
];

const loadingMessages = [
  "Warming up engine assembly...",
  "Applying high-gloss paint...",
  "Polishing carbon fiber...",
  "Finalizing chassis details..."
];
...
const SuperCarManufacturing: React.FC = () => {
    const { t } = useLanguage();
    const [concept, setConcept] = useState(carPresets[0].prompt);
    ...
    useAutoSave('super-car-factory-v5', { concept, scenes, thumbnailUrl, thumbnailPrompt, selectedRatio, youtubeMeta });

    useEffect(() => {
        const handleLoad = (e: any) => {
            const project = e.detail;
            if (project.tool === 'super-car-factory' && project.data) {
    ...
    useEffect(() => {
        if (!thumbnailPrompt && concept) {
            setThumbnailPrompt(`Viral YouTube Thumbnail: ${concept.substring(0, 100)}. Cinematic 3D hyper-realistic render, dramatic lighting, high contrast, 1K resolution, bokeh background.`);
        }
    }, [concept, thumbnailPrompt]);

    const handleClearProject = () => {
        if (!window.confirm("Clear all production data?")) return;
        setConcept(carPresets[0].prompt);
        setScenes([]);
    ...
    const handleGetNewIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateStoryIdeas("Create 5 viral hyper-car manufacturing concepts.", false);
            setStoryIdeas(ideas);
        } catch (err) { setError("Failed to generate ideas."); } finally { setIsGeneratingIdeas(false); }
    };

    const handleUseIdea = (idea: StoryIdea) => {
        setConcept(`Manufacturing Project: ${idea.title}\n\nVision: ${idea.summary}`);
        setThumbnailPrompt(`Viral YouTube Thumbnail: ${idea.title}. Cinematic 3D hyper-realistic render, 8k.`);
        setStoryIdeas([]);
        setScenes([]);
    };
    ...
    const handleGenerateThumbnailIdea = async () => {
        ...
        try {
            ...
            const resp = await ai.models.generateContent({
                ...
                contents: `As a professional YouTube Thumbnail artist, create one highly detailed, cinematic, and viral-worthy image generation prompt for a video about: ${concept}. Focus on composition, dramatic lighting, and high-impact 3D textures. Output ONLY the prompt text, no extra words.`
            });
    ...
    const handleGenerateScript = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `SUPER CAR MANUFACTURING SCRIPT. Concept: ${concept}. Task: Architect exactly ${sceneCount} logical industrial steps. Style: 100% Realistic Industrial Cinematography.`;
    ...
    const handleGenerateShorts = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `SHORT VIRAL CAR PRODUCTION SCRIPT. Concept: ${concept}. Task: Generate EXACTLY 6 senses. Style: Professional industrial 4K cinematography, 100% realistic.`;
    ...
    const handleRenderSingle = async (index: number) => {
        ...
        try {
            ...
            const prompt = `100% Realistic hypercar factory photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. cinematic industrial lighting.`;
    ...
    const handleGenerateSEOInfo = async () => {
        ...
        try {
            const ctx = `Car Project: ${concept}\nScenes:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(concept.substring(0, 50), ctx, 'Car Factory');
            setYoutubeMeta(meta);
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `CarFactory_Sense_${num}.png`;
    ...
    return (
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center text-gray-100 pb-20 animate-fade-in relative">
            <div className="w-full flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-slate-400 uppercase tracking-tighter leading-none">Super Car Factory</h2>
                </div>
                    ...
                    <div className="bg-[#1a1f2e] p-6 rounded-3xl border border-gray-700 h-fit space-y-6 shadow-xl">
                        <div className="bg-black p-4 rounded-xl border border-gray-800 shadow-inner">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <button onClick={handleGetNewIdeas} disabled={isGeneratingIdeas} className="text-[10px] font-black text-red-400 hover:text-red-300 uppercase flex items-center gap-1">
                                    {isGeneratingIdeas ? <Spinner className="h-3 w-3 m-0"/> : <SparklesIcon />} Get New Idea
                                </button>
                            </div>

                            {storyIdeas.length > 0 && (
                                <div className="space-y-2 mb-4 animate-fade-in max-h-48 overflow-y-auto custom-scrollbar">
                                    {storyIdeas.map((idea, idx) => (
                                        <button key={idx} onClick={() => handleUseIdea(idea)} className="w-full p-2 bg-gray-800 border border-gray-700 rounded-xl text-left hover:border-red-500 transition">
                            ...
                            <div className="grid grid-cols-1 gap-2 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                                {carPresets.map((p, i) => (
                                    <button key={i} onClick={() => setConcept(p.prompt)} className={`w-full text-left p-3 rounded-lg border transition-all text-[10px] font-bold uppercase ${concept === p.prompt ? 'bg-red-900/40 border-red-500 text-white shadow-lg' : 'bg-gray-900 border-gray-700 text-gray-400 hover:border-gray-500'}`}>{p.label}</button>
                                ))}
                            </div>
                        </div>

                        <textarea value={concept} onChange={(e) => setConcept(e.target.value)} placeholder="Describe car mission..." className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-4 text-white h-32 resize-none text-sm outline-none focus:ring-2 focus:ring-red-500" />
                            ...
                            <div className="grid grid-cols-5 gap-2">
                                {aspectRatios.map(r => (
                                    <button
                                        ...
                                        className={`p-2 rounded-lg border text-[10px] font-black transition-all flex flex-col items-center gap-1 ${selectedRatio === r.value ? 'bg-red-600 border-red-400 text-white shadow-lg' : 'bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-500'}`}
                                    >
                        ...
                        <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 shadow-inner">
                            ...
                            <input 
                                ...
                                className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-3 text-white font-black text-center text-xl focus:ring-2 focus:ring-red-500 outline-none"
                            />
                        </div>

                        <div className="flex gap-2">
                            <button onClick={handleGenerateScript} disabled={isLoading} className="flex-1 py-4 bg-gradient-to-r from-red-600 to-slate-600 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-xs">
                ...
                <div className="lg:col-span-8 space-y-6 flex flex-col h-full">
                    ...
                    <div className="bg-gray-800/40 p-6 rounded-[2.5rem] border border-gray-700 shadow-2xl animate-fade-in space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                {thumbnailUrl && (<button onClick={() => { const a = document.createElement('a'); a.href = thumbnailUrl; a.download = `Car_Thumbnail.png`; a.click(); }} className="p-2 bg-emerald-600 rounded-lg hover:bg-emerald-500 transition-all shadow-lg"><DownloadIcon /></button>)}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="aspect-video bg-black rounded-2xl border border-gray-700 relative overflow-hidden flex items-center justify-center shadow-inner">
                                ...
                                {isGeneratingThumbnail && (<div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center backdrop-blur-sm"><Spinner className="h-10 w-10 text-red-500 mb-2" /><span className="text-[10px] font-black text-red-400 animate-pulse">RENDERING 1K...</span></div>)}
                            </div>
                            <div className="flex flex-col justify-between">
                                <div className="space-y-4">
                                    <textarea value={thumbnailPrompt} onChange={(e) => setThumbnailPrompt(e.target.value)} placeholder="Customize your viral thumbnail prompt..." className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-3 text-white text-xs h-36 resize-none outline-none focus:ring-1 focus:ring-red-500" />
                                </div>
                                <button onClick={handleGenerateThumbnail} disabled={isGeneratingThumbnail || !thumbnailPrompt} className="w-full mt-4 py-3 bg-gradient-to-r from-red-600 to-red-700 hover:brightness-110 text-white font-black rounded-xl uppercase text-xs shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2">{isGeneratingThumbnail ? <Spinner className="h-4 w-4 m-0" /> : <SparklesIcon />} Generate Thumbnail</button>
                            </div>
                        </div>
                    </div>
                    ...
                    {scenes.length > 0 && (
                        <div className="sticky top-0 z-20 bg-[#0f172a]/90 backdrop-blur-md p-4 rounded-2xl border border-gray-800 shadow-2xl mb-4 flex flex-col md:flex-row justify-between items-center gap-4 animate-fade-in relative overflow-hidden">
                            <div className="flex items-center gap-4">
                                <span className="text-[11px] font-black text-red-400 uppercase tracking-widest">Build Pipeline: {progressPercent}%</span>
                            </div>
                            <div className="flex gap-2 flex-wrap justify-center">
                                ...
                                <button onClick={isRenderingAll ? () => { stopSignal.current = true; setIsRenderingAll(false); } : handleRenderAll} className={`px-4 py-2 rounded-xl font-black text-[10px] uppercase transition shadow-lg flex items-center gap-2 ${isRenderingAll ? 'bg-red-600 text-white' : 'bg-red-700 text-white'}`}>{isRenderingAll ? 'STOP' : 'GEN ALL ART'}</button>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[1200px] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        {scenes.map((scene, idx) => (
                            <div key={idx} ref={el => sceneRefs.current[idx] = el} className="bg-gray-800/20 rounded-2xl border border-gray-700 overflow-hidden flex flex-col group shadow-xl hover:border-red-500/30 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        <>
                                            <img src={scene.imageUrl} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="car" />
                                            <button onClick={() => handleDownloadSingle(scene.imageUrl!, scene.sceneNumber)} className="absolute bottom-2 right-2 p-2 bg-black/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 shadow-xl"><DownloadIcon /></button>
                                        </>
                                    ) : (
                                    ...
                                    <div className="absolute top-4 left-4 bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase shadow-lg">SENSE {scene.sceneNumber}</div>
                                </div>
                                <div className="p-4 flex flex-col flex-grow bg-gradient-to-b from-gray-900 to-black">
                                    ...
                                    <div className="mt-auto flex justify-between items-center pt-3 border-t border-gray-800">
                                        ...
                                        <button onClick={() => handleCopy(JSON.stringify({p: scene.sceneNumber, a: scene.action, pr: scene.consistentContext}, null, 0), `j-${idx}`)} className={`text-[9px] font-black uppercase transition-all flex items-center gap-1 ${copyStatus === `j-${idx}` ? 'text-green-400' : 'text-blue-400 hover:text-blue-300'}`}><JsonIcon /> COMPACT JSON</button>
                                    </div>
                                </div>
                            </div>
                        ))}
...
export default SuperCarManufacturing;
