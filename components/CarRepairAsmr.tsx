...
import { generateConsistentStoryScript, generateImage, generateCarRestorationIdeas, CarIdea, generateYouTubeMetadata, YouTubeMetadata } from '../services/geminiService.ts';
...
const CarRepairAsmr: React.FC = () => {
    const [masterPrompt, setMasterPrompt] = useState('RC Car Repair (ASMR): Restoring a very old, broken miniature remote-controlled truck. ជាប្រភេទ កូន RC តូចៗ ដូចកូនឡាន បញ្ជាដោយ តេលេ។ Focus on satisfying macro hobbyist sounds: tiny screws clinking, cleaning small plastic gears, soldering wires, and a fresh airbrush paint job to make it brand new.');
    const [sceneCount, setSceneCount] = useState(15);
    const [scenes, setScenes] = useState<Scene[]>([]);
    const [isGeneratingScript, setIsGeneratingScript] = useState(false);
    const [isGeneratingIdeas, setIsGeneratingIdeas] = useState(false);
    const [carIdeas, setCarIdeas] = useState<CarIdea[]>([]);
    ...
    useEffect(() => {
        const handleSave = (e: any) => {
            if (e.detail.tool !== 'car-repair-asmr') return;
            const data = {
                id: crypto.randomUUID(),
                timestamp: Date.now(),
                tool: 'car-repair-asmr',
                category: 'vip',
                title: "RC Car Restoration ASMR",
    ...
    const handleGenerateIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateCarRestorationIdeas("Viral RC Car Hobbyist Restoration ASMR: " + masterPrompt);
            setCarIdeas(ideas);
        } catch (err) {
    ...
    const handleUseIdea = (idea: CarIdea) => {
        setMasterPrompt(`RC Car Hobby Project: ${idea.title}\n\nRestoration Plan: ${idea.description}\n\nFocus on satisfying ASMR sounds of miniature restoration: disassembly, ultrasonic cleaning of tiny parts, and custom airbrushing. Keep the original model and color identical throughout.`);
        setCarIdeas([]);
    };
    ...
    const handleGenerateScript = async () => {
        ...
        const voiceoverInstruction = noVoiceover ? "\n\nSTRICT RULE: Do NOT include any spoken dialogue or narration. Focus purely on environmental ASMR sounds: mechanical clicks, liquid dipping, and airbrush hissing." : "";
        
        const consistencyDirective = `
        STRICT VISUAL DIRECTIVE (សូមរក្សារ នៅម៉ូតឡានដ៏ដែល គ្រាន់តែ ធ្វើផុសផុស ដោះដូ គ្រឿងបន្លាស់ នឹងបាញ់ថ្នាំ ពណ៍ មកថ្មីឡើង វិញ):
        1. Keep the exact same RC truck model and design in every scene.
        2. Keep the paint color theme identical.
        3. Only progress the state from broken/dirty to functional/shiny.
        4. PART REPLACEMENT focus: Show old components being swapped for identical brand-new miniature parts.
        `;

        try {
            const result = await generateConsistentStoryScript(
                `RC CAR MODEL RESTORATION ASMR SCRIPT. Context: ${masterPrompt}. Style: 100% Realistic, Professional Hobbyist Workbench Photography. ${consistencyDirective}${voiceoverInstruction}`,
                sceneCount
            );
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic, professional macro photography of an RC car hobbyist workbench, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Detailed textures of tiny screws, plastic gears, wires, tools. Cinematic warm workshop lighting. CRITICAL: Model and color must be identical to Scene 1. (សូមរក្សារ នៅម៉ូតឡានថ្នាំពណ៍ ដ៏ដែល).`;
    ...
    const handleCopyJson = (idx: number) => {
        ...
        const structuredData = {
            ...
            prompt: `100% Realistic, professional macro photography of RC car hobby project, 8k. Action: ${scene.action}. Context: ${scene.consistentContext}.`
        };
        navigator.clipboard.writeText(JSON.stringify(structuredData, null, 2));
        setCopyStatus(`json-${idx}`);
        setTimeout(() => setCopyStatus(null), 2000);
    };
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `RCCar_Sense_${num}.png`;
    ...
    const handleClear = () => {
        setMasterPrompt('RC Car Repair (ASMR): Restoring a very old, broken miniature remote-controlled truck. ជាប្រភេទ កូន RC តូចៗ ដូចកូនឡាន បញ្ជាដោយ តេលេ។ Focus on satisfying macro hobbyist sounds: tiny screws clinking, cleaning small plastic gears, soldering wires, and a fresh airbrush paint job to make it brand new.');
        setScenes([]);
        setCarIdeas([]);
        setNoVoiceover(true);
        setYoutubeMeta(null);
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in">
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                    ...
                    <div className="space-y-4">
                        ...
                        <textarea 
                            ...
                            placeholder="Describe the RC truck restoration journey..."
                    ...
                    <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 space-y-4">
                        <label className="flex items-center gap-3 cursor-pointer group">
                            ...
                            <div className="flex flex-col">
                                <span className="text-sm font-black text-orange-400 uppercase tracking-tighter">No Voiceover (ASMR Only)</span>
                                <span className="text-[9px] text-gray-500 italic">Purely focus on workshop sounds.</span>
                            </div>
                        </label>
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-gray-800/80 p-5 rounded-2xl border border-gray-700 shadow-xl flex flex-col md:flex-row justify-between items-center sticky top-0 z-10 backdrop-blur gap-4">
                        <h3 className="text-xl font-black text-white uppercase tracking-tighter">RC Repair Storyboard ({scenes.length} Senses)</h3>
                        <div className="flex flex-wrap gap-2 justify-center">
                            {scenes.length > 0 && (
                                <>
                                    ...
                                    <button 
                                        onClick={() => {}} 
                                        className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg text-xs font-bold transition shadow-md"
                                    >
                                        <YouTubeIcon /> YouTube Info
                                    </button>
                                </>
                            )}
                        </div>
