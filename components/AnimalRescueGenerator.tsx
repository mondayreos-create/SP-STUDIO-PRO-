...
const AnimalRescueGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [masterPrompt, setMasterPrompt] = useState('Emotional Animal Rescue: A kind person finds an abandoned dog in a dirty alley. Transforming a life from suffering to a loving healthy home. Focus on 100% realism and deep cinematic emotions.');
    ...
    const handleGenerateScript = async () => {
        ...
        try {
            const result = await generateConsistentStoryScript(
                `ANIMAL RESCUE STORYBOARD. Context: ${masterPrompt}. Style: 100% Realistic, Professional Cinematic Emotional Photography.`,
                sceneCount
            );
    ...
    const handleGenerateImage = async (index: number) => {
        ...
        try {
            const prompt = `100% Realistic professional emotional photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. Intense emotion, authentic textures, cinematic lighting. 100% character face consistency. No text.`;
        ...
        scenes.forEach((s, i) => {
            if (s.imageUrl) {
                setTimeout(() => {
                    ...
                    a.href = s.imageUrl!;
                    a.download = `Rescue_Scene_${s.sceneNumber}.png`;
                    a.click();
                }, i * 500);
            }
        });
    ...
    const handleDownload = (url: string, num: number, suffix: string = 'final') => {
        const link = document.createElement('a');
        link.href = url;
        link.download = `Rescue_Sense_${num}_${suffix}.png`;
    ...
    const handleGenerateYouTubeInfo = async () => {
        ...
        try {
            const context = `Rescue Project: ${masterPrompt}\nScenes:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(masterPrompt.substring(0, 50), context, "Animal Rescue");
            setYoutubeMeta(meta);
        } catch (e) {
    ...
    // COMMENT: Removed empty shadowing setCopyStatus function to allow using the state setter from line 98
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                        <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-green-600">Animal Rescue Studio</h2>
                        <textarea 
                            value={masterPrompt}
                        ...
                        />
                        <button onClick={handleGenerateScript} disabled={isGeneratingScript} className="w-full py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-sm">
                    ...
                    {youtubeMeta && (
                        <div className="bg-gray-900 p-6 rounded-2xl border border-red-500/30 animate-fade-in space-y-4 shadow-2xl relative overflow-hidden">
                            ...
                            <div className="space-y-4 text-xs font-bold">
                                <div><label className="text-gray-500 uppercase mb-1 block">Title</label><div className="bg-black/40 p-3 rounded-lg border border-gray-800 text-white">{youtubeMeta.title}</div></div>
                            ...
                            <div key={idx} ref={(el) => (sceneRefs.current[idx] = el)} className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col group hover:border-emerald-500/50 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            {scene.isLoadingImage ? <Spinner className="h-6 w-6 text-green-500" /> : <button onClick={() => handleGenerateImage(idx)} className="px-3 py-1 bg-gray-800 text-[10px] text-gray-400 hover:text-white rounded uppercase font-bold">Render Art</button>}
                                        </div>
                                    )}
                                ...
                                <div className="p-4 flex flex-col flex-grow bg-gradient-to-b from-gray-900 to-black">
                                    ...
                                    <div className="mt-auto flex justify-between pt-3 border-t border-gray-800">
                                        {/* FIX: Corrected handleCopy to handleCopyText. */}
                                        <button onClick={() => handleCopyText(scene.consistentContext, `p-${idx}`)} className="text-[10px] text-gray-500 hover:text-white transition font-black uppercase">PROMPT</button>
                                        {/* FIX: Corrected onClick handler from handleDownloadAll (likely a copy-paste error) to handleDownload. */}
                                        <button onClick={() => handleDownload(scene.imageUrl!, scene.sceneNumber, 'final')} disabled={!scene.imageUrl} className="text-emerald-500 hover:text-emerald-400 transition disabled:opacity-30"><DownloadIcon /></button>
                                    </div>
                                </div>
                            </div>
                        ))}
