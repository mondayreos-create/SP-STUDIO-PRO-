...
const SleepingRoomPro: React.FC = () => {
    ...
    const handleGenerateScript = async () => {
        ...
        try {
            const result = await generateConsistentStoryScript(
                `SLEEPING ROOM RESTORATION. Context: ${masterPrompt}. 0% to 100% transformation with professional crew. Same camera angle.`,
                sceneCount
            );
    ...
    const handleGenerateFrame = async (index: number, type: 'first' | 'mid' | 'last') => {
        ...
        try {
            const stagePrompt = type === 'first' ? "DIRTY 0%" : type === 'mid' ? "CLEANING 75%" : "PERFECT 100%";
            const prompt = `100% Realistic professional interior photography, 8k. Action: ${scene.action}. Stage: ${stagePrompt}. 100% consistency. No text. Same camera position.`;
            ...
            ['First', 'Mid', 'Last'].forEach((slot, j) => {
                ...
                if (url) {
                    setTimeout(() => {
                        ...
                        a.href = url;
                        a.download = `Bedroom_Scene_${s.sceneNumber}_${slot}.png`;
                        a.click();
                    }, (i * 1500) + (j * 500));
                }
            });
    ...
    const handleGenerateYouTubeInfo = async () => {
        ...
        try {
            const context = `Bedroom Project: ${masterPrompt}\nScenes:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(masterPrompt.substring(0, 50), context, "Bedroom Transformation");
            setYoutubeMeta(meta);
        } catch (e) {
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-gray-800/60 p-6 rounded-2xl border border-gray-700 h-fit space-y-6 shadow-xl">
                        <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">បន្ទប់គេង (PRO)</h2>
                        <textarea 
                            value={masterPrompt}
                            onChange={(e) => setMasterPrompt(e.target.value)}
                            placeholder="Describe the sleeping sanctuary..."
                            className="w-full bg-gray-900 border border-gray-600 rounded-xl p-4 text-white h-48 resize-none focus:ring-2 focus:ring-purple-500 outline-none text-sm"
                        />
                        <button onClick={handleGenerateScript} disabled={isGeneratingScript} className="w-full py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-sm">
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="space-y-8 overflow-y-auto max-h-[1000px] custom-scrollbar pr-2 pb-20">
                        {scenes.map((scene, idx) => (
                            <div key={idx} ref={(el) => (sceneRefs.current[idx] = el)} className="bg-gray-900 rounded-3xl overflow-hidden border border-gray-800 shadow-2xl flex flex-col group hover:border-purple-500/50 transition-all duration-300">
                                <div className="p-4 bg-gray-800/50 flex justify-between items-center">
                                    <span className="text-xs font-black text-purple-400">SENSE {scene.sceneNumber}</span>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 bg-black p-1">
                                    {['First', 'Mid', 'Last'].map((slot) => {
                                        const img = (scene as any)[`imageUrl${slot}`];
                                        const loading = (scene as any)[`isLoading${slot}`];
                                        const label = slot === 'First' ? '0% Dirty' : slot === 'Mid' ? '75% Done' : '100% Perfect';
                                        return (
                                            <div key={slot} className="aspect-video bg-gray-900 relative flex items-center justify-center overflow-hidden rounded-xl border border-gray-800">
                                                {img ? (
                                                    <img src={img} className="w-full h-full object-cover" alt={slot} />
                                                ) : (
                                                    <div className="text-center p-2">
                                                        {loading ? <Spinner className="h-6 w-6 text-purple-500" /> : <button onClick={() => handleGenerateFrame(idx, slot.toLowerCase() as any)} className="px-3 py-1 bg-gray-800 text-[9px] text-gray-400 hover:text-white rounded uppercase font-bold">Render {label}</button>}
                                                    </div>
                                                )}
                                                <div className="absolute top-2 left-2 bg-black/70 text-white text-[8px] px-1.5 py-0.5 rounded font-black">{label}</div>
                                            </div>
                                        );
                                    })}
                                </div>
