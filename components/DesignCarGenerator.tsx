...
const carPresets = [
    { label: "Classic Muscle Rescue", prompt: "Restoring a 1969 Mustang: From a rust-covered shell in a barn to a deep metallic cherry red masterpiece. High-detail engine polishing and interior leather restoration. 100% Realistic 8k." },
    { label: "Desert Find Supercar", prompt: "Abandoned Ferrari in the desert: Sand-covered and faded. Satisfying power wash, carbon fiber panel replacement, and Rosso Corsa paint restoration. 100% Realistic 8k." },
    { label: "Retro JDM Transformation", prompt: "Old Nissan Skyline R34: Cleaning deep grime, custom wide-body kit installation, and Bayside Blue high-gloss paint job. 100% Realistic 8k." },
    { label: "Barn Find 1950s Truck", prompt: "Rusty 1954 Chevy Truck: Complete frame-off restoration. Sandblasting the metal, wood bed restoration, and a vintage matte teal finish. 100% Realistic 8k." },
    { label: "Flood Damaged Luxury", prompt: "Flood-damaged Rolls Royce: Removing mud, intense interior detailing, electronics repair, and a perfect black diamond paint polish. 100% Realistic 8k." }
];

const loadingMessages = [
  "Mixing automotive paint...",
  "Applying high-gloss finish...",
  "Polishing chrome details...",
  "Rendering carbon fiber..."
];
...
const RenderLoadingOverlay: React.FC<{ progress: number; messageIndex: number }> = ({ progress, messageIndex }) => (
    <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center z-10 animate-fade-in p-4 text-center">
        ...
        <span className="text-[8px] font-black text-orange-500 uppercase tracking-widest">{Math.round(progress)}% Complete</span>
    </div>
);
...
const DesignCarGenerator: React.FC = () => {
    const { t } = useLanguage();
    ...
    useAutoSave('design-car-factory-v1', { concept, scenes, thumbnailUrl, thumbnailPrompt, selectedRatio });

    useEffect(() => {
        if (!thumbnailPrompt && concept) {
            setThumbnailPrompt(`Viral YouTube Thumbnail: ${concept.substring(0, 100)}. Cinematic 3D hyper-realistic car restoration render, dramatic lighting, 8k.`);
        }
    ...
    const handleGetNewIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateStoryIdeas("Create 5 viral car restoration or custom design concepts.", false);
            setStoryIdeas(ideas);
        } catch (err) { setError("Failed to generate ideas."); } finally { setIsGeneratingIdeas(false); }
    };

    const handleUseIdea = (idea: StoryIdea) => {
        setConcept(`Car Design Project: ${idea.title}\n\nVision: ${idea.summary}`);
    ...
    const handleGenerateThumbnailIdea = async () => {
        ...
        try {
            ...
            const resp = await ai.models.generateContent({
                ...
                contents: `As a professional YouTube Thumbnail artist, create one highly detailed, cinematic, and viral-worthy image generation prompt for a car restoration video: ${concept}. Focus on composition, glossy reflections, dramatic lighting, and high-impact 3D textures. Output ONLY the prompt text, no extra words.`
            });
    ...
    const handleGenerateScript = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `CAR DESIGN AND RESTORATION SCRIPT. Concept: ${concept}. Task: Architect exactly ${sceneCount} logical restoration steps. Style: 100% Realistic Industrial Cinematography.`;
    ...
    const handleGenerateShorts = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `SHORT VIRAL CAR RESTORATION SCRIPT. Concept: ${concept}. Task: Generate EXACTLY 6 senses. Style: Professional industrial 4K cinematography, 100% realistic.`;
    ...
    const handleRenderSingle = async (index: number) => {
        ...
        try {
            ...
            const prompt = `100% Realistic hypercar restoration photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. cinematic lighting.`;
    ...
    const handleGenerateSEOInfo = async () => {
        ...
        try {
            ...
            const meta = await generateYouTubeMetadata(concept.substring(0, 50), ctx, 'Car Restoration');
            setYoutubeMeta(meta);
        } catch (err) { setError("Failed to generate ideas."); } finally { setIsGeneratingMeta(false); }
    };
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `CarDesign_Sense_${num}.png`;
        ...
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center text-gray-100 pb-20 animate-fade-in">
            <div className="w-full flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-red-400 uppercase tracking-tighter leading-none">Car Design Studio</h2>
                </div>
                    ...
                    <div className="bg-[#1a1f2e] p-6 rounded-3xl border border-gray-700 h-fit space-y-6 shadow-xl">
                        <div className="bg-black p-4 rounded-xl border border-gray-800 shadow-inner">
                            <div className="flex justify-between items-center mb-3">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Design Blueprints</label>
                                <button onClick={handleGetNewIdeas} disabled={isGeneratingIdeas} className="text-[10px] font-black text-orange-400 hover:text-orange-300 uppercase flex items-center gap-1">
                            ...
                            {storyIdeas.length > 0 && (
                                <div className="space-y-2 mb-4 animate-fade-in max-h-48 overflow-y-auto custom-scrollbar">
                                    {storyIdeas.map((idea, idx) => (
                                        <button key={idx} onClick={() => handleUseIdea(idea)} className="w-full p-2 bg-gray-800 border border-gray-700 rounded-xl text-left hover:border-orange-500 transition">
                            ...
                            <div className="grid grid-cols-1 gap-2 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                                {carPresets.map((p, i) => (
                                    <button key={i} onClick={() => setConcept(p.prompt)} className={`w-full text-left p-3 rounded-lg border transition-all text-[10px] font-bold uppercase ${concept === p.prompt ? 'bg-orange-900/40 border-orange-500 text-white shadow-lg' : 'bg-gray-900 border-gray-700 text-gray-400 hover:border-gray-500'}`}>{p.label}</button>
                                ))}
                            </div>
                        </div>

                        <textarea value={concept} onChange={(e) => setConcept(e.target.value)} placeholder="Describe car mission..." className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-4 text-white h-32 resize-none text-sm outline-none focus:ring-2 focus:ring-orange-500" />
                        ...
                        <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 shadow-inner">
                            ...
                            <input type="number" min="1" max="100" value={sceneCount} onChange={e => setSceneCount(Math.max(1, Math.min(100, parseInt(e.target.value) || 1)))} className="w-full bg-[#0f172a] border border-gray-800 rounded-xl p-3 text-white font-black text-center text-xl focus:ring-2 focus:ring-orange-500 outline-none" />
                        </div>

                        <div className="flex gap-2">
                            <button onClick={handleGenerateScript} disabled={isLoading} className="flex-1 py-4 bg-gradient-to-r from-orange-600 to-red-600 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-xs">
                ...
                <div className="lg:col-span-8 space-y-6 flex flex-col h-full">
                    ...
                    <div className="bg-gray-800/40 p-6 rounded-[2.5rem] border border-gray-700 shadow-2xl animate-fade-in space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="aspect-video bg-black rounded-2xl border border-gray-700 relative overflow-hidden flex items-center justify-center shadow-inner">
                                ...
                                {isGeneratingThumbnail && (<div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center backdrop-blur-sm"><Spinner className="h-10 w-10 text-orange-500 mb-2" /><span className="text-[10px] font-black text-orange-400 animate-pulse">RENDERING 4K...</span></div>)}
                            </div>
                            <div className="flex flex-col justify-between">
                                <div className="space-y-4">
                                    <textarea value={thumbnailPrompt} onChange={(e) => setThumbnailPrompt(e.target.value)} placeholder="Customize your viral thumbnail prompt..." className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-3 text-white text-xs h-24 resize-none outline-none focus:ring-1 focus:ring-orange-500" />
                                </div>
                    ...
                    {scenes.length > 0 && (
                        <div className="sticky top-0 z-20 bg-[#0f172a]/90 backdrop-blur-md p-4 rounded-2xl border border-gray-800 shadow-2xl mb-4 flex flex-col md:flex-row justify-between items-center gap-4 animate-fade-in relative overflow-hidden">
                            <div className="flex items-center gap-4">
                                <span className="text-[11px] font-black text-orange-400 uppercase tracking-widest">Build Pipeline: {progressPercent}%</span>
                            </div>
                            <div className="flex gap-2 flex-wrap justify-center">
                                ...
                                <button onClick={isRenderingAll ? () => { stopSignal.current = true; setIsRenderingAll(false); } : handleRenderAll} className={`px-4 py-2 rounded-xl font-black text-[10px] uppercase transition shadow-lg flex items-center gap-2 ${isRenderingAll ? 'bg-red-600 text-white' : 'bg-orange-600 text-white'}`}>{isRenderingAll ? 'STOP' : 'GEN ALL ART'}</button>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[1200px] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        {scenes.map((scene, idx) => (
                            <div key={idx} ref={el => sceneRefs.current[idx] = el} className="bg-gray-800/20 rounded-2xl border border-gray-700 overflow-hidden flex flex-col group shadow-xl hover:border-orange-500/30 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        <>
                                            ...
                                            <button onClick={() => handleDownloadSingle(scene.imageUrl!, scene.sceneNumber)} className="absolute bottom-2 right-2 p-2 bg-black/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-orange-600 shadow-xl"><DownloadIcon /></button>
                                        </>
                                    ) : (
                                        <div className="text-center p-4">
                                            {!scene.isLoading && <button onClick={() => handleRenderSingle(idx)} className="text-[10px] font-black uppercase text-gray-500 hover:text-orange-400 border border-gray-800 px-4 py-2 rounded-full transition-colors">Render Part</button>}
                                        </div>
                                    )}
                                    {scene.isLoading && <RenderLoadingOverlay progress={simulatedProgress[idx] || 0} messageIndex={simulatedMessageIdx[idx] || 0} />}
                                    <div className="absolute top-4 left-4 bg-orange-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase shadow-lg">SENSE {scene.sceneNumber}</div>
                                </div>
...
export default DesignCarGenerator;