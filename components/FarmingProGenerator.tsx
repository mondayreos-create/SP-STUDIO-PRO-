...
const farmPresets = [
    { label: "Vertical Strawberry", icon: "🍓", prompt: "Professional Indoor Vertical Strawberry Farm: Robot arms gently picking ripe strawberries from high-tech glowing towers, pink lighting, clear nutrient tubes, ultra-detailed fruit texture, 8k." },
    { label: "Robotic Honey", icon: "🐝", prompt: "Automated Beehive Sanctuary: High-tech autonomous beehives in a wildcard meadow. Robotic cold-extraction of golden honey flowing into glass containers. Cinematic sunlight, 100% realistic." },
    { label: "Smart Fish Farm", icon: "🐟", prompt: "Autonomous Aquaculture: Giant circular ocean pens with robotic feeding and cleaning subs. Clear blue water, schools of fish, high-tech maritime farming." },
    { label: "Hydroponic Lettuce", icon: "🥬", prompt: "Massive vertical hydroponic lettuce farm. Automated nutrient spray, purple LED growth lights, and robotic harvesting rollers in a sterile facility." },
    { label: "Drone Vineyard", icon: "🍇", prompt: "High-tech vineyard management. Autonomous drones flying over rows of grapes, laser-trimming vines, and robotic picking baskets under a golden v." },
    { label: "Automated Dairy", icon: "🐄", prompt: "Futuristic robotic dairy farm. Automated milking stations, laser-guided cow health monitoring, and high-speed robotic bottling of fresh milk." },
    { label: "Mushroom Reactor", icon: "🍄", prompt: "Mushroom Bio-reactor chamber. Glowing blue climate-controlled pods growing exotic glowing mushrooms. Robotic arms monitoring humidity and spores in a dark, high-tech industrial facility. 100% Realistic." },
    { label: "Saffron Greenhouse", icon: "🌸", prompt: "High-precision saffron cultivation. Robotic tweezers delicately harvesting purple saffron flowers, high-detail macro photography, controlled greenhouse setting." },
    { label: "Seaweed Factory", icon: "🌿", prompt: "Off-shore automated seaweed farm. Underwater robotic cutters harvesting kelp, high-speed drying conveyors, and clear blue ocean visuals." },
    { label: "Vertical Herb Wall", icon: "🌱", prompt: "Modular indoor herb garden. High-speed planting robots, liquid nutrient tubes, and automated packaging of fresh basil and mint." },
    { label: "Robotic Orchard", icon: "🍎", prompt: "Smart apple orchard. Quadruped robots climbing ladders to pick fruit, laser-guided sorting by size and color, and automated crate loading." },
    { label: "Smart Poultry", icon: "🐣", prompt: "High-tech automated poultry habitat. Climate controlled domes, robotic feeding tracks, and AI health monitoring for thousands of birds." },
    { label: "Algae Biofuel", icon: "🧪", prompt: "Futuristic algae farm. Transparent green bioreactors, high-pressure extraction tubes, and high-speed centrifugal processing for biofuel." },
    { label: "Tomato Greenhouse", icon: "🍅", prompt: "Precision tomato farming. Robotic arms picking vine-ripened tomatoes, automated pest-detecting drones, and high-detail fruit textures." },
    { label: "Rice Swarm Robotics", icon: "🌾", prompt: "Smart rice cultivation. Swarm of small aquatic robots planting seedlings, laser-guided water level management, and autonomous harvesters." },
    { label: "Wheat Swarm", icon: "🚜", prompt: "Massive wheat field harvest. A synchronized swarm of 20 autonomous combine harvesters working in perfect formation at golden hour." },
    { label: "Coffee Bean Process", icon: "☕", prompt: "High-tech coffee processing. Optical sorting robots selecting perfect beans, automated fermentation tanks, and high-speed roasting machines." },
    { label: "Tea Leaf Robots", icon: "🍵", prompt: "Precision tea harvesting on mountain slopes. Micro-robots picking only the top two leaves, automated solar drying trays, and misty atmosphere." },
    { label: "Vanilla Greenhouse", icon: "🍦", prompt: "Controlled vanilla orchid farming. Robotic hand-pollination of orchid flowers, high-detail macro shots of green pods, and automated curing rooms." },
    { label: "Truffle Lab", icon: "💎", prompt: "Underground smart truffle cultivation. Precision moisture sensors, robotic root inoculation, and high-tech ultrasonic harvest detection." }
];

const loadingMessages = [
  "Mixing nutrient solutions...",
  "Powering up robotic harvesters...",
  "Rendering 8K leaf textures...",
  "Finalizing harvest logic..."
];
...
const RenderLoadingOverlay: React.FC<{ progress: number; messageIndex: number }> = ({ progress, messageIndex }) => (
    <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center z-10 animate-fade-in p-4 text-center">
        ...
        <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em]">{Math.round(progress)}% Complete</span>
    </div>
);
...
const FarmingProGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [masterPrompt, setMasterPrompt] = useState(farmPresets[0].prompt);
    ...
    useAutoSave('farming-pro-v5', { masterPrompt, sceneCount, scenes, thumbnailUrl, thumbnailPrompt, selectedRatio, youtubeMeta });

    useEffect(() => {
        const handleLoad = (e: any) => {
            const project = e.detail;
            if (project.tool === 'farming-pro' && project.data) {
    ...
    useEffect(() => {
        if (!thumbnailPrompt && masterPrompt) {
            setThumbnailPrompt(`Viral YouTube Thumbnail for Agri-Tech: ${masterPrompt.substring(0, 100)}. 100% Realistic, high contrast, cinematic agricultural photography, 1K resolution.`);
        }
    ...
    const handleGetNewIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateStoryIdeas("Create 5 futuristic farm automation concepts.", false);
            setStoryIdeas(ideas);
        } catch (err) { setError("Failed to generate ideas."); } finally { setIsGeneratingIdeas(false); }
    };

    const handleUseIdea = (idea: StoryIdea) => {
        setMasterPrompt(`Farming Project: ${idea.title}\n\nVision: ${idea.summary}`);
        setThumbnailPrompt(`Viral YouTube Thumbnail for Agri-Tech: ${idea.title}. 100% Realistic, 8k.`);
        setScenes([]);
        setStoryIdeas([]);
    };
    ...
    const handleGenerateThumbnailIdea = async () => {
        ...
        try {
            ...
            const resp = await ai.models.generateContent({
                ...
                contents: `As a professional YouTube Thumbnail artist, create one highly detailed, cinematic, and viral-worthy image generation prompt for an advanced farming/agri-tech video: ${masterPrompt}. Focus on organic textures, robotic high-tech systems, and dramatic sunlight. Output ONLY the prompt text, no extra words.`
            });
    ...
    const handleGenerateScript = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `MODERN AUTOMATIC FARMING PRODUCTION. Context: ${masterPrompt}. Breakdown into exactly ${sceneCount} industrial steps. Style: 100% Realistic agricultural photography.`;
    ...
    const handleGenerateShorts = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `SHORT VIRAL AGRI-TECH PRODUCTION SCRIPT. Concept: ${masterPrompt}. Task: Generate EXACTLY 6 senses for a high-retention sequence. Style: Professional industrial 4K cinematography, 100% realistic. Focus on the 'Satisfaction' aspect for TikTok/Reels.`;
    ...
    const handleRenderScene = async (index: number) => {
        ...
        try {
            ...
            const renderPrompt = `100% Realistic professional agricultural photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. No text. Cinematic natural lighting.`;
    ...
    const handleGenerateSEOInfo = async () => {
        ...
        try {
            const ctx = `Farming Project: ${masterPrompt}\nScenes:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(masterPrompt.substring(0, 50), ctx, 'Agri-Tech');
            setYoutubeMeta(meta);
    ...
    const handleCopyAllPrompts = () => {
        ...
        handleCopyText(text, 'all-prompts');
    };
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `Farm_Scene_${num}.png`;
    ...
    return (
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center animate-fade-in text-gray-100 pb-20 relative">
            <div className="w-full flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-green-500 uppercase tracking-tighter leading-none">Farming Pro Hub</h2>
                </div>
                <button onClick={() => { if(window.confirm("Reset?")) { setScenes([]); setYoutubeMeta(null); setThumbnailUrl(null); } }} className="p-2.5 bg-red-950/20 text-red-500 border border-red-900/30 rounded-xl hover:bg-red-900 transition-all px-4 py-2 font-bold uppercase text-xs">Clear Project</button>
            </div>
            
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {/* Left side: Blueprints & Controls */}
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-[#1a2e26] p-6 rounded-[2.5rem] border border-gray-700 h-fit space-y-6 shadow-xl">
                        <div className="bg-black p-4 rounded-xl border border-gray-800 shadow-inner">
                            <div className="flex justify-between items-center mb-3">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Farm Blueprints</label>
                            ...
                            <div className="grid grid-cols-1 gap-2 mb-6 max-h-96 overflow-y-auto custom-scrollbar pr-1">
                                {farmPresets.map((opt) => (
                                    <button key={opt.label} onClick={() => setMasterPrompt(opt.prompt)} className={`p-3 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-3 border ${masterPrompt === opt.prompt ? 'bg-emerald-900/40 border-emerald-500 text-white shadow-lg' : 'bg-gray-900 border-gray-700 text-gray-400 hover:border-gray-500'}`}>
                        ...
                        <textarea value={masterPrompt} onChange={(e) => setMasterPrompt(e.target.value)} placeholder="Describe farm vision..." className="w-full bg-[#0f172a] border border-gray-700 rounded-2xl p-4 text-white text-xs h-32 resize-none outline-none transition-all placeholder-gray-800" />
                        ...
                        <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 shadow-inner">
                            ...
                            <input type="number" min="1" max="50" value={sceneCount} onChange={e => setSceneCount(Math.max(1, Math.min(50, parseInt(e.target.value) || 1)))} className="w-full bg-[#0f172a] border border-gray-700 rounded-lg p-2 text-center font-bold text-sm" />
                        </div>

                        <div className="flex gap-2">
                             <button onClick={handleGenerateScript} disabled={isLoading} className="flex-1 py-4 bg-gradient-to-r from-emerald-600 to-green-700 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-xs">
                ...
                <div className="lg:col-span-8 space-y-6 flex flex-col h-full">
                    ...
                    <div className="bg-gray-800/40 p-6 rounded-[2.5rem] border border-gray-700 shadow-2xl animate-fade-in space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                {thumbnailUrl && (<button onClick={() => { const a = document.createElement('a'); a.href = thumbnailUrl; a.download = `Farm_Thumbnail.png`; a.click(); }} className="p-2 bg-emerald-600 rounded-lg hover:bg-emerald-500 transition-all shadow-lg"><DownloadIcon /></button>)}
                            </div>
                        </div>
                            ...
                            <div className="aspect-video bg-black rounded-2xl border border-gray-700 relative overflow-hidden flex items-center justify-center shadow-inner">
                                ...
                                {isGeneratingThumbnail && (
                                    <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center backdrop-blur-sm">
                                        <Spinner className="h-10 w-10 text-emerald-500 mb-2" />
                                        <span className="text-[10px] font-black text-emerald-500 animate-pulse">RENDERING 1K...</span>
                                    </div>
                                )}
                            </div>
                            <div className="flex flex-col justify-between">
                                <div className="space-y-4">
                                    <textarea value={thumbnailPrompt} onChange={(e) => setThumbnailPrompt(e.target.value)} placeholder="Customize your viral thumbnail prompt..." className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-3 text-white text-xs h-36 resize-none outline-none focus:ring-1 focus:ring-emerald-500" />
                                </div>
                                <button onClick={handleGenerateThumbnail} disabled={isGeneratingThumbnail || !thumbnailPrompt} className="w-full mt-4 py-3 bg-gradient-to-r from-emerald-600 to-green-600 hover:brightness-110 text-white font-black rounded-xl uppercase text-xs shadow-xl flex items-center justify-center gap-2">{isGeneratingThumbnail ? <Spinner className="h-4 w-4 m-0" /> : <SparklesIcon />} Generate Thumbnail</button>
                            </div>
                        </div>
                    </div>
                    ...
                    {scenes.length > 0 && (
                        <div className="sticky top-0 z-20 bg-[#0f172a]/90 backdrop-blur-md p-4 rounded-2xl border border-gray-800 shadow-2xl mb-4 flex flex-col md:flex-row justify-between items-center gap-4 animate-fade-in relative overflow-hidden">
                            ...
                            <div className="flex gap-2 flex-wrap justify-center">
                                <button onClick={handleGenerateSEOInfo} disabled={isGeneratingMeta} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded-xl text-[10px] font-black uppercase transition shadow-lg flex items-center gap-2">{isGeneratingMeta ? <Spinner className="h-3 w-3 m-0" /> : <YouTubeIcon />} {t('btn_seo_kit')}</button>
                    ...
                    {youtubeMeta && (
                        <div className="bg-gray-900 p-6 rounded-[2.5rem] border border-red-500/30 animate-fade-in space-y-6 shadow-2xl relative overflow-hidden mb-6">
                            ...
                            <div className="space-y-4">
                                <div><label className="text-gray-500 uppercase block mb-1">Viral Title</label><div className="bg-black/40 p-3 rounded-lg border border-gray-800 text-white text-sm font-bold flex justify-between items-center">{youtubeMeta.title} <button onClick={() => handleCopyText(youtubeMeta.title, 'yt-t')} className="text-cyan-400 hover:text-white transition">{copyStatus === 'yt-t' ? <AnimatedCheckmark /> : 'Copy'}</button></div></div>
                                <div><label className="text-gray-500 uppercase block mb-1">SEO Description</label><div className="bg-black/40 p-3 rounded-lg border border-gray-800 text-gray-300 text-xs whitespace-pre-wrap leading-relaxed max-h-40 overflow-y-auto custom-scrollbar italic">{youtubeMeta.description}</div></div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="text-gray-500 uppercase block mb-1">Hashtags</label><div className="bg-black/40 p-3 rounded-lg border border-gray-800 text-blue-400 flex justify-between items-center"><span className="truncate">{youtubeMeta.hashtags.join(' ')}</span> <button onClick={() => handleCopyText(youtubeMeta.hashtags.join(' '), 'yt-h')} className="text-cyan-400 hover:text-white transition">{copyStatus === 'yt-h' ? <AnimatedCheckmark /> : 'Copy'}</button></div></div>
                                    <div><label className="text-gray-500 uppercase block mb-1">Keywords / Tags</label><div className="bg-black/40 p-3 rounded-lg border border-gray-800 text-emerald-400 flex justify-between items-center"><span className="truncate">{youtubeMeta.keywords.join(', ')}</span> <button onClick={() => handleCopyText(youtubeMeta.keywords.join(', '), 'yt-k')} className="text-cyan-400 hover:text-white transition">{copyStatus === 'yt-k' ? <AnimatedCheckmark /> : 'Copy'}</button></div></div>
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        {scenes.map((scene, idx) => (
                            <div key={idx} ref={el => sceneRefs.current[idx] = el} className="bg-gray-800/20 rounded-2xl border border-gray-700 overflow-hidden flex flex-col group shadow-xl hover:border-emerald-500/30 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        <>
                                            <img src={scene.imageUrl} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="farm" />
                                        ...
                                        <div className="text-center p-4">
                                            {!scene.isLoading && <button onClick={() => handleRenderScene(idx)} className="text-[10px] font-black uppercase text-gray-500 hover:text-white border border-gray-800 px-4 py-2 rounded-full transition-colors">Render Part</button>}
                                        </div>
                                    )}
                                ...
                                <div className="p-4 bg-gradient-to-b from-gray-900 to-black rounded-b-2xl flex flex-col flex-grow">
                                    ...
                                    <div className="mt-auto flex justify-between items-center pt-3 border-t border-gray-800">
                                        ...
                                        <button onClick={() => handleCopyText(JSON.stringify({p: scene.sceneNumber, a: scene.action, pr: scene.consistentContext}, null, 0), `json-${idx}`)} className={`text-[9px] font-black uppercase tracking-tighter transition-all ${copyStatus === `json-${idx}` ? 'text-green-400 scale-110' : 'text-blue-400 hover:text-blue-300'}`}>{copyStatus === `json-${idx}` ? '✓ DONE' : 'COMPACT JSON'}</button>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            {error && <div className="fixed bottom-4 left-4 right-4 z-50 p-4 bg-red-900/90 border border-red-700 text-red-200 rounded-xl shadow-2xl text-center font-bold text-sm">{error}</div>}
        </div>
    );
};

export default FarmingProGenerator;
