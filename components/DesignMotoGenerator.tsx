...
const motoPresets = [
    { label: "Vintage Cafe Racer", prompt: "Restoring an old rusty 1970s motorcycle into a premium cafe racer. Transforming 0% scrap metal to 100% polished chrome and custom leather. 100% Realistic 8k." },
    { label: "Modern Sportbike", prompt: "Complete restoration of a crashed Ducati Panigale. Installing new carbon fiber fairings, titanium exhaust, and a high-gloss signature red finish. High-tech garage setting." },
    { label: "Harley Bobber Build", prompt: "Customizing a rusty cruiser into a minimalist matte black bobber. Stripping the frame, installing a solo spring seat, and wide tires. Satisfying industrial ASMR." },
    { label: "Classic Scooter Refresh", prompt: "Restoring an abandoned 1960s Vespa scooter. From peeling paint and rust to a vibrant high-gloss pastel mint finish with white wall tires." }
];

const loadingMessages = [
  "Stripping old paint...",
  "Polishing engine parts...",
  "Applying high-gloss lacquer...",
  "Rendering chrome reflections..."
];
...
const RenderLoadingOverlay: React.FC<{ progress: number; messageIndex: number }> = ({ progress, messageIndex }) => (
    <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center z-10 animate-fade-in p-4 text-center">
        <div className="relative mb-6">
            <div className="w-12 h-12 border-4 border-violet-500/20 rounded-full"></div>
        ...
        <span className="text-[8px] font-black text-violet-500 uppercase tracking-widest">{Math.round(progress)}% Complete</span>
    </div>
);
...
const DesignMotoGenerator: React.FC = () => {
    const { t } = useLanguage();
    const [concept, setConcept] = useState(motoPresets[0].prompt);
    ...
    useAutoSave('design-moto-factory-v1', { concept, sceneCount, scenes, thumbnailUrl, thumbnailPrompt, selectedRatio });

    useEffect(() => {
        if (!thumbnailPrompt && concept) {
            setThumbnailPrompt(`Viral YouTube Thumbnail: ${concept.substring(0, 100)}. Cinematic 3D hyper-realistic motorcycle restoration render, dramatic lighting, 8k.`);
        }
    }, [concept, thumbnailPrompt]);

    const handleClearProject = () => {
        if (!window.confirm("Clear all production data?")) return;
        setConcept(motoPresets[0].prompt);
        setScenes([]);
    ...
    const handleGetNewIdeas = async () => {
        ...
        setError(null);
        try {
            const ideas = await generateStoryIdeas("Create 5 viral motorcycle restoration or custom design concepts.", false);
            setStoryIdeas(ideas);
        } catch (err) { setError("Failed to generate ideas."); } finally { setIsGeneratingIdeas(false); }
    };

    const handleUseIdea = (idea: StoryIdea) => {
        setConcept(`Moto Design Project: ${idea.title}\n\nVision: ${idea.summary}`);
        setThumbnailPrompt(`Viral YouTube Thumbnail: ${idea.title}. Cinematic 3D hyper-realistic motorcycle render, 8k.`);
        setStoryIdeas([]);
        setScenes([]);
    };
    ...
    const handleGenerateThumbnailIdea = async () => {
        ...
        try {
            ...
            const resp = await ai.models.generateContent({
                ...
                contents: `As a professional YouTube Thumbnail artist, create one highly detailed, cinematic, and viral-worthy image generation prompt for a motorcycle restoration video: ${concept}. Focus on composition, mechanical detail, chrome reflections, and high-impact industrial lighting. Output ONLY the prompt text, no extra words.`
            });
    ...
    const handleGenerateScript = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `MOTORCYCLE DESIGN AND RESTORATION SCRIPT. Concept: ${concept}. Task: Architect exactly ${sceneCount} logical restoration steps. Style: 100% Realistic Industrial Cinematography.`;
    ...
    const handleGenerateShorts = async () => {
        ...
        setScenes([]);
        try {
            const prompt = `SHORT VIRAL MOTO RESTORATION SCRIPT. Concept: ${concept}. Task: Generate EXACTLY 6 senses. Style: Professional industrial 4K cinematography, 100% realistic.`;
    ...
    const handleRenderSingle = async (index: number) => {
        ...
        try {
            ...
            const prompt = `100% Realistic high-end motorcycle restoration photography, 8k. Action: ${scene.action}. Environment: ${scene.consistentContext}. cinematic garage lighting.`;
    ...
    const handleGenerateSEOInfo = async () => {
        ...
        try {
            const ctx = `Moto Project: ${concept}\nScenes:\n${scenes.map(s => s.action).join('\n')}`;
            const meta = await generateYouTubeMetadata(concept.substring(0, 50), ctx, 'Motorcycle Restoration');
            setYoutubeMeta(meta);
    ...
    const handleDownloadSingle = (url: string, num: number) => {
        ...
        link.href = url;
        link.download = `Moto_Sense_${num}.png`;
    ...
    return (
        <div className="w-full max-w-7xl mx-auto p-4 flex flex-col items-center text-gray-100 pb-20 animate-fade-in">
            <div className="w-full flex justify-between items-center mb-6">
                <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-indigo-500 uppercase tracking-tighter leading-none">Moto Design Hub</h2>
                    ...
                    <div className="bg-[#1a1f2e] p-6 rounded-3xl border border-gray-700 h-fit space-y-6 shadow-xl">
                        <div className="bg-black p-4 rounded-xl border border-gray-800 shadow-inner">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <button onClick={handleGetNewIdeas} disabled={isGeneratingIdeas} className="text-[10px] font-black text-violet-400 hover:text-violet-300 uppercase flex items-center gap-1">
                            ...
                            {storyIdeas.length > 0 && (
                                <div className="space-y-2 mb-4 animate-fade-in max-h-48 overflow-y-auto custom-scrollbar pr-1">
                                    {storyIdeas.map((idea, idx) => (
                                        <button key={idx} onClick={() => handleUseIdea(idea)} className="w-full p-2 bg-gray-800 border border-gray-700 rounded-xl text-left hover:border-violet-500 transition">
                            ...
                            <div className="grid grid-cols-1 gap-2 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                                {motoPresets.map((p, i) => (
                                    <button key={i} onClick={() => setConcept(p.prompt)} className={`w-full text-left p-3 rounded-lg border transition-all text-[10px] font-bold uppercase ${concept === p.prompt ? 'bg-violet-900/40 border-violet-500 text-white shadow-lg' : 'bg-gray-900 border-gray-700 text-gray-400 hover:border-gray-500'}`}>{p.label}</button>
                                ))}
                            </div>
                        </div>

                        <textarea value={concept} onChange={(e) => setConcept(e.target.value)} placeholder="Describe moto restoration..." className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-4 text-white text-sm h-32 resize-none focus:ring-1 focus:ring-violet-500 outline-none" />
                            ...
                            <div className="grid grid-cols-5 gap-2">
                                {aspectRatios.map(r => (
                                    <button
                                        ...
                                        className={`p-2 rounded-lg border text-[10px] font-black transition-all flex flex-col items-center gap-1 ${selectedRatio === r.value ? 'bg-violet-600 border-violet-400 text-white shadow-lg' : 'bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-500'}`}
                                    >
                        ...
                        <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 shadow-inner">
                            ...
                            <input type="number" min="1" max="100" value={sceneCount} onChange={e => setSceneCount(Math.max(1, Math.min(100, parseInt(e.target.value) || 1)))} className="w-full bg-[#0f172a] border border-gray-800 rounded-xl p-3 text-white font-black text-center text-xl focus:ring-2 focus:ring-violet-500 outline-none" />
                        </div>

                        <div className="flex gap-2">
                            <button onClick={handleGenerateScript} disabled={isLoading} className="flex-1 py-4 bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-xs">Architect</button>
                            <button onClick={handleGenerateShorts} disabled={isLoading} className="flex-1 py-4 bg-gradient-to-r from-fuchsia-600 to-violet-700 text-white font-black rounded-xl shadow-xl transition transform active:scale-95 disabled:opacity-50 uppercase tracking-widest text-xs">Fast Shorts</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="bg-gray-800/40 p-6 rounded-[2.5rem] border border-gray-700 shadow-2xl animate-fade-in space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                {thumbnailUrl && (<button onClick={() => { const a = document.createElement('a'); a.href = thumbnailUrl; a.download = `Moto_Thumbnail.png`; a.click(); }} className="p-2 bg-emerald-600 rounded-lg hover:bg-emerald-500 transition shadow-lg"><DownloadIcon /></button>)}
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="aspect-video bg-black rounded-2xl border border-gray-700 relative overflow-hidden flex items-center justify-center shadow-inner">
                                ...
                                {isGeneratingThumbnail && (<div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center backdrop-blur-sm"><Spinner className="h-10 w-10 text-violet-500 mb-2" /><span className="text-[10px] font-black text-violet-500 animate-pulse">RENDERING 4K...</span></div>)}
                            </div>
                            <div className="flex flex-col justify-between">
                                <div className="space-y-4">
                                    <textarea value={thumbnailPrompt} onChange={(e) => setThumbnailPrompt(e.target.value)} placeholder="Viral thumbnail prompt..." className="w-full bg-[#0f172a] border border-gray-700 rounded-xl p-3 text-white text-xs h-24 resize-none outline-none focus:ring-1 focus:ring-violet-500" />
                                </div>
                                <div className="flex gap-2 mt-4">
                                    <button onClick={handleGenerateThumbnail} disabled={isGeneratingThumbnail || !thumbnailPrompt} className="flex-1 py-3 bg-gradient-to-r from-violet-600 to-indigo-600 hover:brightness-110 text-white font-black rounded-xl uppercase text-xs shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2">{isGeneratingThumbnail ? <Spinner className="h-4 w-4 m-0" /> : <SparklesIcon />} Generate Thumbnail</button>
                                </div>
                    ...
                    {scenes.length > 0 && (
                        <div className="sticky top-0 z-20 bg-[#0f172a]/90 backdrop-blur-md p-4 rounded-2xl border border-gray-800 shadow-2xl mb-4 flex flex-col md:flex-row justify-between items-center gap-4 animate-fade-in relative overflow-hidden">
                            <div className="flex items-center gap-4">
                                <span className="text-[11px] font-black text-violet-400 uppercase tracking-widest">Build Pipeline: {progressPercent}%</span>
                            </div>
                            <div className="flex gap-2 flex-wrap justify-center">
                                ...
                                <button onClick={isRenderingAll ? () => { stopSignal.current = true; setIsRenderingAll(false); } : handleRenderAll} className={`px-4 py-2 rounded-xl font-black text-[10px] uppercase transition shadow-lg flex items-center gap-2 ${isRenderingAll ? 'bg-red-600 text-white' : 'bg-violet-600 text-white'}`}>{isRenderingAll ? 'STOP' : 'GEN ALL ART'}</button>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-h-[1200px] overflow-y-auto custom-scrollbar pr-2 pb-20">
                        {scenes.map((scene, idx) => (
                            <div key={idx} ref={el => sceneRefs.current[idx] = el} className="bg-gray-800/20 rounded-2xl border border-gray-700 overflow-hidden flex flex-col group shadow-xl hover:border-violet-500/30 transition-all duration-300">
                                <div className="aspect-video bg-black relative flex items-center justify-center overflow-hidden">
                                    {scene.imageUrl ? (
                                        <>
                                            <img src={scene.imageUrl} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="moto" />
                                            <button onClick={() => handleDownloadSingle(scene.imageUrl!, scene.sceneNumber)} className="absolute bottom-2 right-2 p-2 bg-black/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-violet-600 shadow-xl"><DownloadIcon /></button>
                                        </>
                                    ) : (
                                        <div className="text-center p-4">
                                            {!scene.isLoading && <button onClick={() => handleRenderSingle(idx)} className="text-[10px] font-black uppercase text-gray-500 hover:text-violet-400 border border-gray-800 px-4 py-2 rounded-full transition-colors">Render Part</button>}
                                        </div>
                                    )}
                                    {scene.isLoading && <RenderLoadingOverlay progress={simulatedProgress[idx] || 0} messageIndex={simulatedMessageIdx[idx] || 0} />}
                                    <div className="absolute top-4 left-4 bg-violet-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase shadow-lg">SENSE {scene.sceneNumber}</div>
                                </div>
...
export default DesignMotoGenerator;